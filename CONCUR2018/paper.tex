...
\titlerunning{Formal Synthesis of Digital Controllers for Observable Continuous Plants}
...
\authorrunning{Cattaruzza, David, Abate, Kroening}

\Copyright{Cattaruzza, David, Abate, Kroening}%mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/

\subjclass{Computer systems organization~Embedded software}% mandatory: Please choose ACM 2012 classifications from https://www.acm.org/publications/class-2012 or https://dl.acm.org/ccs/ccs_flat.cfm . E.g., cite as "General and reference $\rightarrow$ General literature" or \ccsdesc[100]{General and reference~General literature}. 


%\email{alessandro.abate@cs.ox.ac.uk}
%\email{dario.cattaruzza@cs.ox.ac.uk}
%\email{cristina.david@cs.ox.ac.uk}
%\author{Daniel Kroening}\email{kroening@cs.ox.ac.uk}
...
%\begin{keyword}        
\keywords {Digital Control; A/D converters; Control System Synthesis; Safety Analysis; Quantization Errors; Observer-based Controller. Sampling.}
%\end{keyword}                             
...
discretized model and to find candidate solutions fast, or to refine this model on demand.  
Moreover, in order to improve our chances of finding a solution
even when considering coarse abstractions of the system, 
 in each iteration  we 
%of the CEGIS loop we use matrix eigen-decomposition to 
optimize our candidate.
%
...
(i.e., without abstraction refinement or optimization).  We~demonstrate the
...
%% that fail the specification on a very large number of inputs),
...
\item We automatically generate \emph{correct-by-construction} digital
  observer-based feedback controllers that guarantee a given safety property
  ...
  full state feedback controllers (hence the need for the abstraction/refinement procedure). 
%
\item  Our synthesis addresses challenges created by digitalization errors, 
 ...
 sampling and processing times, all of which introduce control errors in real
 implementations.
 %
...
can only measure $\vec{y}$, the state $\vec{x}$ cannot be measured directly.}, namely 
%
...
\end{equation}
%
namely as the union of $k$-reach sets over $n$ iterations. 
%
...
We note that the introduction of an observer doubles the size of the model w.r.t.
a full state controller, which will have an average polynomial, worst case
exponential time increase in the sysnthesis engine.
...
$t=T$, and then use abstraction~\cite{JSS14,cattaruzza2015unbounded} to overapproximate $\hat{X}_n$.
...
\end{align*}
%
Jury's criterion~\cite{fadali} uses these coefficients to ensure that no root of the
...
\phi_\mathit{stability} \wedge \phi_\mathit{input} \wedge \phi_\mathit{init}$.
...
Next, we describe the salient blocks in the CEGIS-OR loop of Figure~\ref{fig:CEGIS-OR}.
...
which is characterised by $\max\{|\tan^{-1}\left(\frac{im(\lambda)}{re(\lambda)}\right)|, \lambda
...
smaller rotation, which facilitates meeting the specification
$\phi_\mathit{safety}$.
...
([xshift=5em]synth.north)
   node[align=left,yshift=-0.7cm,xshift=.5cm] {{$\phi$}\\{$\phi_\mathit{init}^K$}} %{$\phi_\mathit{safety}$}\\ 
...
%
\hline
\# & \multicolumn{1}{|c|}{Benchmark} & \multicolumn{1}{|c|}{p} & \multicolumn{1}{|c|}{No.} & \multicolumn{4}{|c|}{CEGIS\cite{DBLP:conf/cav/AbateBCCDKKP17}} & \multicolumn{4}{|c|}{CEGIS-OR} \\
...
is impossible for us to differentiate these from the case where a controller
does not exist.

In order to show the improvements due to our optimization-enhanced
CEGIS, we compared our approach against the CEGIS approach with
abstraction refinement described
in~\cite{DBLP:conf/cav/AbateBCCDKKP17}.  We note that,
while~\cite{DBLP:conf/cav/AbateBCCDKKP17} presents two approaches, one
of them (referred to as the naive approach
in~\cite{DBLP:conf/cav/AbateBCCDKKP17}) cannot handle the continuous
time case and thus we can only compare against the faster, abstraction-based
one (referred to as CEGIS~\cite{DBLP:conf/cav/AbateBCCDKKP17} in Table~\ref{tab:cegis_results}).  %% Furthermore, since it is in general slower than the abstraction
%% refinement version, the latter is a more representative benchmark).
For the majority of our benchmarks, we observe that our approach is
significantly faster than CEGIS~\cite{DBLP:conf/cav/AbateBCCDKKP17}.  %% We
%% further note that the number of times CEGIS times out is much larger
%% than CEGIS-OR which results in more found solutions.
...
meaning that the CEGIS~\cite{DBLP:conf/cav/AbateBCCDKKP17}
loop would require many iterations.
...
%% both approaches..}.
These improvements are due to the interplay between Abstraction
Refinement and Optimization in CEGIS-OR.  While abstraction refinement
enables us to start with coarse abstractions that are only refined on
demand, the optimization allows to locally improve candidate solutions
and avoid entire iterations of the
CEGIS~\cite{DBLP:conf/cav/AbateBCCDKKP17} loop (which would have also
required more refined abstractions).
...
%% improvements.
While some systems might still require a very precise abstraction
resulting in many refinements (this is generally the cause of
time-outs in CEGIS-OR), others allow us to find solutions with rather
coarse abstractions (this is reflected in the smaller minimum times
obtained with CEGIS-OR).

%% (SAT has a worst case exponential time complexity, and
%% typical polynomial time, so higher order systems will show larger
%% differences).
