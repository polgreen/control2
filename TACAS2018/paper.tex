...
phases:
\begin{itemize}
\item {\sc Abstract}: constructs an unsound abstract model that over-approximates
the dynamics of the system.
\note{what does it mean to be over-approximating but unsound?}
\item {\sc synthesize}: generates a candidate controller and observer.
  ...
  %% these do not necessarily meet any portion
...
\item {\sc model}: refines the abstraction needed by {\sc Abstract}
  in order to reduce the probability of failure in the next iteration.
  \note{{\sc abstract} doesn't need an abstraction.}
  \note{how about optimization?}
\item {\sc verify}: verifies the behaviour of the candidate controller and observer
  against a sound over-approximation of the concrete dynamics of the system for an infinite space over an infinite time horizon. If the verification fails, produces a counterexample
  describing the reason for failure.
\end{itemize}

\note{this needs to be integrated in the above enumeration}
Our optimization procedure aims at reducing the
size of the reach space of the closed loop system while maintaining other
verification properties. \note{what are other verification properties?
if you mean safety and stability say so.}
%% The first one aims at making the system as
...
\begin{itemize}
\item[1.] We start with simpler (i.e. imprecise) abstractions that get
...
\note{the next paragraph doesn't sound right, we need to separate the notions
  of abstraction and specification. they should not be the same. for now, just give an
intuition and we'll think more...}

The CEGIS-OR loop for this approach is described next.  \note{give an
  intuition for what $\phi_{static}$ and $\phi_{synth}$ represent,
  e.g. the system's abstraction + stability spec + safety spec...}
Note that we make use of two abstractions corresponding to two
specifications: $\phi_{synth}$ used by the synthesiser and
$\phi_{static}$ which is given to the synthesis refinement loop. These
abstractions will be refined independently, so we will not detail what
each will contain at a given stage. We only note that at all times
$\phi_{static} \models \phi_{synth}$.
