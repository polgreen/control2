...
%
$k \in \mathbb N$ is a discrete counter and $\vec{x}_{0}=\vec{x}(0)$ is the
initial state.  In this work we raise the standard assumption that
...
certain properties of the plant hold at any given real time.  In order to do
so, we accelerate the dynamics of the plant according to the model semantics
in Sec.~\ref{sec:model_semantics} and compute the state $\vec{x}_T$ at a
given time $T$,
%
