...
\phi_{dynamics}^{T_d}&:= \mat{A}_d, \mat{B}_d, \mat{C}_d
\end{align*}
The first one defines the word length of the digital controller ($\mathbb{R}_{\langle C \rangle}$) and its effect on the system dynamics.
The second one defines the sampling time ($T_d$) and its effect on the system dynamics (as per Section \ref{sec:continuous_time_accel}).
...
\end{align*}
These specifications will vary as the loop progresses.  We only note that at all times $\phi_{static} \models \phi_{synth}$.
