...
systems (i.e., where all the states are known) modeled in discrete time,
and furthermore explores only a limited set of properties that engineers
...
effect on the output. A plant model that minimizes the difference
...
Our goal in this work is to synthesize both $\mat{K}_{\langle C \rangle}$ and $\mat{L}_{\langle C \rangle}$, 
...
and approximations due to digitalization.  
...
Since we are interested in synthesizing controllers with observers, 
...
The benefit of this representation is that it has no multiplications, which are expensive to handle by SAT solvers (used by the synthesizer). 
%
...
\label{eq:set_time}
\end{equation}
where $0\leq P_s \leq 1$ is the desired portion of the target within which the output must remain after $t_s$.  For the proof see Appendix~\ref{sec:proofs}
\end{theorem}
...
engine. In each iteration, an inductive synthesizer produces a
...
{\sc synthesize} phase checks $\phi$ by using the plant precision
...
may therefore find solutions that the synthesizer cannot, due to the use of limited precision.
If we cannot optimize the candidate  we signal the {\sc Abstract} phase to 
use the updated $\phi$ and request {\sc Synthesize} to produce a new
...
approximations in the synthesizer that would prevent a true candidate from being found
...
There is still a need to extend CEGIS-OR to MIMO models and to analyze more general properties, 
...
and the overall behavior of the resulting system is said to have closed loop dynamics.
...
(this simple sufficient condition can be generalized, however this is
