...
\resizebox{1.0\textwidth}{!}{
 ...
 \caption{Closed-loop digital control system with an observer, where the controller and the observer (bottom part) are implemented in a discrete domain (quantities within this domain are identified by the index $\langle C \rangle$), whereas the plant is continuous \label{fig:observersystem2}}
\end{figure*}
...
The state of the model $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)+\mat{B}\vec{u}(t)$ at time $T$ given a parametric input $\vec{u}$ is 
 ...
 then $\vec{x}_T=A_d^{\frac{T}{T_d}}\vec{x}+(\mat{I}-\mat{A}_d^{\frac{T}{T_d}}\mat{B}_d\vec{u}) \text{ with } T \in \mathbb{R}_0^+$ \note{here there must be an x that is actually a x0?} (where $\mat{A}_d^{\frac{T}{T_d}} = e^{log(\mat{A}_d) \frac{T}{T_d}} = e^{\mat{A} T}$) is the state of the system at time $T$.
 \end{corollary}
\begin{proof}
See Appendix~\ref{sec:proofs}
\end{proof}
...
maintaining other properties (safety, stability, and response time).
...
\note{do you mean small plant precision?}
...
\paragraph{\underline{{\sc synthesize}}} generates a candidate controller $\mat{K}$
...
system's abstraction as well as optimizes the current candidate
solution.
%
%% When a counterexample is found, we add it to $\phi_{synth} = \phi_{synth} \wedge \phi_{safety}^k$, 
%% where k is the iteration found for the counterexample.  
First, the system's abstraction must be refined in order to eliminate
the newly received counterexample. For this purpose,
we increase the time horizon $k$ for the
safety specification so that it captures the violating iteration
(for the given initial state) and adjust $\phi$:
$\phi = \phi \wedge \phi_{safety}^k$.
...
for the plant's discretization.
...
If we cannot optimize the candidate  we signal the {\sc Abstract} phase to potentially 
increase the precision and 
%phase to
%refine $\phi$ and
request {\sc Synthesise} to produce a new
candidate.  Otherwise the optimized candidate solution is given back to
{\sc Verify Synthesis}.
\note{when is the precision increased in abstract?}
