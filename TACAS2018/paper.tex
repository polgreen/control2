...
\footnote{This restricts the observability of the state-space $\vec{x}$, since we can only measure $\vec{y}$. 
\note{what does this mean? do you assume observability? don't you need that if you later compute the observable canonical form?}}:
...
\end{equation}
%
\note{what is the dimension of C, D?}
...
We assume that the system has no inputs, so its dynamics are described by $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)$.
 ...
 \note{matrices S are not defined}
...
\note{so: the statement of this theorem is a standard result in the control literature - why is this a new theorem? we do not prove anything new here.}
 \end{theorem}
 \begin{corollary}
 If $\mat{J}$ is diagonal and there exists a discrete dynamics matrix for a sampling time $T_d$ such that $A_d=e^{\mat{A} T_d}$, then $\vec{x}_T=A_d^{\frac{T}{T_d}}\vec{x} \text{ with } T \in \mathbb{R}^+_0$ is the state of the system at time $t=T$. \note{there needs to be an x0}
 \note{so I assume this is the new contribution?}
 \end{corollary}
\begin{proof}
...
underscore~${\langle C \rangle}$ (see Figure \ref{fig:observersystem2}).
...
1  & DC Motor          & 2 & 100 & 1.3\,s & 26.9\,s& \xmark (22)& 1.1\,s &   3.8\,s &   \xmark (2)\\
2  & Helicopter        & 3  & 100 & 57.2\,s & \xmark (54) & \xmark (54) & 7.1\,s  &   18.0\,s & \xmark (9)\\
3  & Inverted Pendulum & 4 & 100 &   9.7\,s & \xmark &\xmark () & 12.0\,s  &  18.1\,s &  \xmark ()\\
4  & Magnetic Pointer  & 2  & 100 & 5.7\,s & 229.9\,s & \xmark (38) & 3.9\,s  &  11.4\,s &  \xmark (5) \\
%5 & Magnetic Suspension          & 2 & 1 & 12.2\,s& 0.6\,s & & & N/A  &   N/A &   N/A\\
5  & Pendulum          & 2 & 100 & 14.0\,s & 2.0\,s & \xmark ()&0.4\,s  &   4.9\,s & \xmark  (6)\\
...
Experimental results. Minimum, Maximum and Median times for standard CEGIS vs CEGIS-OR on multiple benchmarks. \xmark~indicates timeout, with the number of benchmarks that timed out in parenthesis.}
\end{table}
