...
As we want to use Jury's criterion~\cite{fadali}\footnote{More details on Jury's criterion is in Appendix~\ref{sec:closed_stability}}, which is expressed 
by an easy algebraic formula, we transform the model into a representation that contains the desired coefficients expressed as sums of values (no multiplications).  Such representations are called Controllable and Observable Canonical Forms~\cite{Astrom08} \footnote{See parts~\ref{sec:reachable} and~\ref{sec:observable} in the Appendix}.  In this section we explore the implications of using these canonical forms alongside an FWL representation.
