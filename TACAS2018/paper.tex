...
\resizebox{1.0\textwidth}{!}{
 \begin{tikzpicture}[scale=0.6,-,>=stealth',shorten >=.2pt,auto,
  ...
  \matrix[nodes={draw, fill=none, shape=rectangle, minimum height=.2cm, minimum width=.2cm, align=center}, row sep=.6cm, column sep=.6cm] {
   ...
   \& complexnode/.pic={ 
      \node[rectangle,dashed,draw,minimum width=8cm,minimum height=3.5cm,label=\textbf{Model}] (plant)  at ([yshift=-.5cm]dac.center) {};
 ...
 \caption{Closed-loop digital control system with observer \note{it is unclear what we mean by digital here, we have only discretised time so far.}\label{fig:observersystem}}
\end{figure*}
...
\note{What exactly is a ``witness''? Do you mean ``the solution at that time''?}
 \end{theorem}
 %% We can apply Th.~\ref{eq:continuous_tube_dyn} to discretized systems without inputs in order to
 %% obtain a witness ..
 %
 \note{proof? So what is the use of this theorem? How does it help to address the issue discussed at the beginning of the section?}  
 \begin{corollary}
...
%\end{proof}
 ...
 inputs, described by $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)+\mat{B}\vec{u}(t)$ \note{confusing: if the input is parametric, it should not depend on time}. 
...
\vec{x}_T'&=\mat{A}\mat{A}_T\vec{x}_0 + (\mat{I}-\mat{A}_T)\mat{B}\vec{u} : \forall t \leq T,\ \vec{u}(t)=\vec{u} 
 ...
 \note{Just write u, w/o time dependence.}
\end{theorem}
 \begin{corollary}
...
Previous studies~\cite{gangli1} showed that the FWL affects the poles and
zeros positions, degrading the closed-loop dynamics, causing steady-state
...
\todo{Start by saying what stability means and that we must have the eigenalues
  inside the unity circle and that we want to use Jury's criterion.
  Then, explain what the challenges are for observed systems and how they are overcome. Potentially, the overcoming can be moved to appendix.}
