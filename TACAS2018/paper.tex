...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\todo{How does fit in the description at the beginning of sec 3.3?}
...
\todo{equations \eqref{eq:feedback_sampled_cont} and
  \eqref{eq:feedback_cont} are in the appendix so it's preferable to not
refer to them so directly.}
...
\label{eq:set_time}
\end{equation}
where $0\leq p_s \leq 1$ is the desired percentage of the target within \todo{typo?} to remain after $t_s$.
\end{theorem}
%\begin{proof}
...
to assert this specification. \todo{Not clear to me what ``this specification'' refers to.}
...
\label{eq:of_bounds}
\end{equation}
\todo{what happened to u when going from \eqref{eq:observer_LTI_cf}?}
%
%Note that we use the bounds of the input after the transformation by $\mat{B}$ since the matrix may reverse the bounds. 
%
\todo{I thought \eqref{eq:of_bounds} was the safety spec. Why is it solved?}
Equation~\eqref{eq:of_bounds} can be solved using linear programming as it
describes a polyhedral region in $\mathbb{R}^p$.  The second \todo{second?} step is
...
$\mat{E}\mat{T}\vec{x}_0<\mat{f}$ \todo{ET? f?}.  The presence of $\mat{T}$ is because
