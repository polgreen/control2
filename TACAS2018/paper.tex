...
\author{Dario Cattaruzza, Alessandro Abate, Daniel Kroening}%\email{alessandro.abate@cs.ox.ac.uk}
...
evolving over a continuous state space and accounting for errors caused by
the digitization effects of the controller.  We use multi-staged
counterexample-guided inductive synthesis (CEGIS) with four phases: 1)~We
create a modular abstraction of the model and feed it to a two stage synthesiser
to reduce computational cost.  2)~We use the coarsest level of
abstraction to synthesise an approximate observer-based controller using
a Bounded Model Checker(BMC), and then  3)~We feed the imprecise result to
a second synthesiser based on eigendecomposition which uses a more precise
abstraction to refine the result.
The BMC is computationally more expensive, but able to reason
about more complex models, whereas the eigensolver is fast but requires a
reference candidate solution close to the desired result to guide the synthesis.
The intuition is that because our solutions form sets of connected spaces, 
the desired solution lies in the vicinity of a [possibly] unsuitable candidate
found by the first synthesiser.  4)~The refined candidate solution is still
unsound.  Safety and other design properties are then verified using continuous
and discrete-time reachability analysis tools.  If either verification
fails, a counterexample is provided to the modelling phase, which will refine
the abstraction based on the type of counter-example and feed it back to
the synthesisers.  The process iterates until a safe controller is obtained.
We demonstrate the practical value of this approach by automatically
synthesizing safe observer-based controllers for physical plant models from
the digital control literature.
%
\end{abstract}
...
The coarse synthesiser uses a low resolution discretized model of a 
...
Due to either numerical imprecision or overapproximations, the candidate
solution presented by the first synthesiser will likely not meet the spec. A 
second synthesiser, with a much more precise refinement including a 
mapping between the real and transformed spaces will create a candidate
solution that is more adequate to the real model. This second stage uses
perturbation analysis and therefore requires an initial solution to operate.
If the second synthesiser is unable to find a candidate solution, it will cue
the modeller to supply a finer abstraction to the coarse synthesiser which
will in turn provide a better candiate to the second stage.
...
space/time.  This stage uses abstraction refinement when needed to ensure
that rejected candidates are not being discarded due to the
over-approximation.  The nature of the counterexamples varies depending on
...
%
where $ \vec{x}_0 \in X_0$ and $\forall k\geq 0, \vec{x}_{k+1} = \tau(\vec{x}_k,\vec{u}_k) $, 
where 
%
\begin{equation}\label{equ:reachtraj}
\tau(\vec{x}_k,\vec{u}_k) = 
\big( \mat{A}\vec{x}_k +
\mat{B}\vec{u}_k \mid \mat{G}\vec{x}_k \leq \vec{h} \wedge \vec{u}_k \in U \big) \;. 
\end{equation}
...
\begin{align*}
x(kT_s)=x_k \text{ and } y(kT_s) = y_k, \forall k
\end{align*}
...
$M$.  Let also $\mathbb{R}_{\langle M,[E] \rangle}$ be a floating point
...
\begin{enumerate}
\item Truncation\\
Let $\mathcal{F}_{\langle M,F \rangle}(\cdot) : \mathbb{R} \rightarrow \mathbb{R}_{\langle M,F \rangle}$
\begin{align*}
...
alongside the vector multiplication $\mat{K}\vec{x}$. Furthermore, 
...
%\mat{K}' \in \mathbb{R}_{\langle C \rangle}$. 
 ...
 \begin{tikzpicture}[scale=0.3,->,>=stealth',shorten >=.2pt,auto, semithick, initial text=, ampersand replacement=\&,]
  ...
  \&\& complexnode/.pic={ 
	...
	label={\sc Model~~~~~~~~~~~},] (model) {};
     \node[minimum width=2cm, minimum height=0.6cm, fill=gray!20] (model2) at ([xshift=-3cm]model.center) {{\sc 3. Refine}\\{\sc Synthesis}};
     \node[minimum width=2cm, minimum height=0.6cm, fill=gray!20] (model3) at ([xshift=0cm]model.center) {{\sc 4. Refine}\\{\sc Abstraction}};
     \node[minimum width=2cm, minimum height=0.6cm, fill=gray!20] (model4) at ([xshift=3cm]model.center) {{\sc 5. Sample}};
   } ;\\
   ...
   complexnode/.pic={ 
	...
	label={\sc Verify~~~~~~~~~~~},] (verif) {};
     \node[minimum width=2cm, minimum height=0.6cm, fill=gray!20] (verif1) at ([xshift=-3cm]verif.center) {{\sc 5. Verify}\\{\sc Synthesis}};
     \node[minimum width=2cm, minimum height=0.6cm, fill=gray!20] (verif2) at ([xshift=0cm]verif.center) {{\sc 6. Discrete}\\{\sc Time}};
   ...
   \path[-]
    (model3.north) edge node[align=center,xshift=0.8cm] {$k$,$X_0'$} (aux2.north)
    (aux2.west) edge (aux1.east)
    (aux3.west) edge (aux2.east)
    (model4.north) edge node[align=center,xshift=0.5cm] {$T_s$} (aux3.north);
   \path
    ([yshift=2em]synth.east) edge node[xshift=-0.5em,align=center] {$\mat{K}$,$\mat{L}$} ([yshift=2em]verif1.west)
    ([yshift=-2em]verif1.west) edge node {C-ex} ([yshift=-2em]synth.east)
    (verif1.east) edge node {SAT} (verif2.west)
    (verif2.north) edge node[xshift=.8cm] {C-ex} (model3.south)
    (verif2.east) edge node {SAT} (verif3.west)
    (verif3.north) edge node {C-ex} (model4.south)
    (aux1.south) edge node {$\phi$} (model1.north)
    (model3.west) edge node {$\phi_\Lambda$} (model2.east)
    (model4.west) edge node {$\phi_\Lambda$} (model3.east)
    ([xshift=-5em]fp.north) edge node[align=center]  {SAT/\\C-ex} ([xshift=-5em]verif2.south)
    ([xshift=-5em]sv.north) edge node[align=center]  {SAT/\\C-ex} ([xshift=-5em]verif3.south)
    ...
    ([xshift=-5em]bmc.north) edge node[align=center]  {SAT/\\C-ex} ([xshift=-5em]verif1.south)
    (verif) edge node {PASS} (done)
    ([xshift=5em]synth.south) edge node[align=center] {Inputs} ([xshift=5em]gp.north)
    ([xshift=-5em]gp.north) edge node[align=center] {UNSAT/\\$\mat{K}$,$\mat{L}$} ([xshift=-5em]synth.south)
    ([xshift=5em]model1.south) edge node[align=center,xshift=-.65cm] {$\langle P \rangle$} ([xshift=5em]synth.north) node[align=left,yshift=-0.7cm,xshift=1.3cm] {{$\phi_{stability}(P_a,P_l,\overline{\lambda})$}\\{$\phi_{safety}(\eta,k)$}\\ {$\phi_{init}^K$}}
   ([xshift=-5em]synth.north) edge node[align=center] {UNSAT} ([xshift=-5em]model1.south);
   \path[->]
    ([xshift=-5em]verif1.north) edge node[align=center]  {$\mat{K}$,$\mat{L}$} ([xshift=-5em]model2.south)
    ([xshift=5em]model2.south) edge node[align=center]  {$\mat{K}'$,$\mat{L}'$} ([xshift=5em]verif1.north)
    (model2.west) edge node[align=center] {$\overline{\lambda},\langle C \rangle$} (model1.east)
    ([yshift=1em]model1.east) edge node[align=center] {$\phi_{static}$} ([yshift=1em]model2.west);
