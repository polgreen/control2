...
and is characterised by a gain matrix $\mat{K}_{\langle C \rangle}$ that influences the closed-loop dynamics.  
...
%  defined as the difference between the real output $\vec{y}_k$ and estimated one $\hat{\vec{y}_k}$, 
  whereas $\mat{L}_{\langle C \rangle}$ is an error gain matrix that amplifies the output estimation error. 
...
\end{proof}
 ...
 We will assume $\mat{A}$ is invertible when there are geometric multiplicities $>1$.
...
\vec{x}_T'&=\mat{A}\mat{A}_T\vec{x}_0 + (\mat{I}-\mat{A}_T)\mat{B}\vec{u} \text{ where } \forall t \leq T,\ \vec{u}(t)=\vec{u} 
 ...
 is the state of the model $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)+\mat{B}\vec{u}(t)$ at time $T$ given a parametric input $\vec{u}$.
 \end{theorem}
 \begin{corollary}
 If $\mat{J}$ is diagonal and there exists a discrete dynamics matrix for a sampling time $T_d$ such that $A_d=e^{\mat{A} T_d}$, then $\vec{x}_T=A_d^{\frac{T}{T_d}}\vec{x}+(\mat{I}-\mat{A}_d^{\frac{T}{T_d}}\mat{B}\vec{u}) \text{ with } T \in \mathbb{R}^+$ is the state of the system at time $T$.
 \end{corollary}
\begin{proof}
See Appendix~\ref{sec:proofs}
\end{proof}
...
We will assume $\mat{A}$ is invertible, since otherwise $B_T$ would have to be calculated by integration which is impractical.
...
Everything that happens in the digital world takes place at times $t=kT_s, k \in \mathbb{N}$ (discrete time progression),
and the output of the result is delayed at the DAC when it is fed back into the continuous time domain at time ($t=kT_s+t_d$).
...
\end{align}
where $\vec{y}_{t_d}$ is the output of the system with delay $t_d$, and $\norm{\vec{x}}_1=\sum_{i=0}^{size(\vec{x})} |\vec{x}_i|$ is the $l_1$ norm (which we extend to sets as $\norm{X}_1 = sup \{\norm{\vec{x}}_1 \mid \vec{x} \in X\}$). 
\end{theorem}

\begin{proof}
See Appendix~\ref{sec:proofs}
\end{proof}
We over-approximate the effects of this vector by introducing a noise signal at the output $\nu_t=[-q_t, q_t]$ and use the dynamics of the system without delay.  This noise will be small when the delay is much smaller than the sampling time.
...
sound controllers.
The models we consider have two sources of error that are due to Finite Word Length (FWL)
representation.
...
In this paper, we are interested in asymptothic stability, where the output is expected to
...
by an easy algebraic formula, we transform the model into a representation that contains the desired coefficients expressed as sums of values (no multiplications).  Such representations are called Controllable and Observable Canonical Forms~\cite{Astrom08} \footnote{See~\ref{sec:reachable} and~\ref{sec:observable}}.  In this section we explore the implications of using these canonical forms alongside an FWL representation.
