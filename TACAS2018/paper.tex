...
\note{which are reflected in the canonical form of the resulting model or something else to
...
be true.  We must therefore find a suitable solution to maintain this separation
\note{which separation?}.
...
%
\begin{equation}
P_o'=\sum_{j=0}^p c_j\overline{\lambda'}^{p-j}s^j, \text{ where }  \overline{\lambda'}=(1+\delta_o)\overline{\lambda} \text{ and } c_j=l_j-a_j.
\end{equation}
%
...
%
\begin{equation}
P_r'=\sum_{j=0}^p c_j\overline{\lambda'}^{p-j}s^j, \text{ where }  \overline{\lambda'}=(1+\delta_c)\overline{\lambda} \text{ and } c_j=k_j-a_j.
\end{equation}
