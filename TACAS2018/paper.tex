...
\resizebox{1.0\textwidth}{!}{
 ...
 \caption{Closed-loop digital control system with an observer, where the controller and the observer (bottom part) are implemented in a discrete domain (identified by the subindex $\langle C \rangle$), whereas the plant is continuous \label{fig:observersystem2}}
\end{figure*}
...
The state of the model $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)+\mat{B}\vec{u}(t)$ at time $T$ given a parametric input $\vec{u}$ is 
 ...
 \end{align}
 where $\forall t \leq T,\ \vec{u}(t)=\vec{u}$, and $\mat{A}_T$ as in \eqref{eq:continuous_tube_dyn}
 ...
 then $\vec{x}_T=A_d^{\frac{T}{T_d}}\vec{x}+(\mat{I}-\mat{A}_d^{\frac{T}{T_d}}\mat{B}_d\vec{u}) \text{ with } T \in \mathbb{R}_0^+$ (where $\mat{A}_d^{\frac{T}{T_d}} = e^{log(\mat{A}_d) \frac{T}{T_d}} = e^{\mat{A} T}$) is the state of the system at time $T$.
 \end{corollary}
\begin{proof}
See Appendix~\ref{sec:proofs}
\end{proof}
...
We transform the model into a representation that contains the coefficients of the characteristic polynomial (used by Jury's criterion) expressed as sums of controller and plant values (with no multiplications, which are expensive for the SAT solver).  
...
New problems arise due to the FWL effects on these matrices.  The Canonical
form of the resulting system is expressed by the formula:
%
\begin{align}
...
\end{align}
where $\mat{T}_c$ and $\mat{T}_o$ trasform the system into Canonical Form~\cite{Astrom08}, $\vec{x}'$ and $\vec{x}_e'$ are the state of the system and the error of the observer evaluated in the transformed domain \note{this statement will not be understood} and
...
maintaining other verification properties (safety, stability, and response time).
...
Throughout the CEGIS-OR loop we make use of a specification $\phi$
which encompasses the abstraction of the system's dynamics.
Note that, given that the system's abstraction gets refined
in each iteration, $\phi$ will also be adjusted appropriately.
Initially, $\phi$ is given by: 
...
{\sc synthesise} phase checks $\phi$ by using the plant's precision
...
\paragraph{1. {\sc abstract}} computes an initial version of $\phi$
and decides the sample time $T_d$,
the plant's precision $\mathbb{R}_{\langle P \rangle}$ and the controller's precision
...
\begin{itemize}
\item Select a sample time ($T_d$) that minimizes the magnitudes of the matrices used in both the
implementation and the abstraction. This ensures that a small bit vector size is required for
the synthesis process.
\note{implementation and abstraction?}
...
\item Create a stability specification $\phi_{stability}$ based on the characteristic polynomials, perturbation
error ($\mat{A}_e$,$\mat{B}_e$), and response time %%(P_a,P_l,\overline{\lambda})$
as described in Section~\ref{sec:transientspecification}.
\note{could not find perturbation error anywhere}
...
%% \item Under certain circumstances (i.e. when {\sc optimize} did not manage to improve the s) When a counterexample is found, we add it to $\phi_{synth} = \phi_{synth} \wedge \phi_{safety}^k$, 
%% where k is the iteration found for the counterexample.  The ${\sc Abstract}$
%% phase will then construct a abstraction (in the form of a program) that meets
%% this property (unsoundly since the program uses FWL).
\end{itemize}
\note{$\phi_{safety}^k$ is undefined!!!}
%
%\item
...
which may choose to increase the plant's precision or return a synthesis failure.
If {\sc synthesize} generates a candidate solution, it is fed to the {\sc verify} chain in order to validate it.
%
%\item
\paragraph{3. {\sc Verify Synthesis}} verifies that the candidate controller
and observer obey $\phi$ (remember that we now use
$\mathbb{R}_{\langle C \rangle}$ for the controller's representation
and reals for the plant).  If the verification fails, the candidate is
fed to the {\sc Optimise} phase.  If the candidate is valid, we feed
it to the next {\sc verify} stage, {\sc Discrete Time}.

%\item
\paragraph{4. {\sc Optimise}} optimises the failed candidate solution.
We seek to minimize the distance of the farthest point in the
...
verified against the safety specification $\phi_{safety}$ and the
objective $e$ is measured (Equation \eqref{eq:objective}).  If for
...
%% At each optimizing iteration, the modeller updates $\phi_{static} = \phi_{static} \wedge \phi_{objective}$% \wedge \phi_{safety}^{k'} $ where $k' \geq k$.

If we cannot optimize the candidate, we signal the {\sc Abstract} phase to
refine $\phi$ and request {\sc Synthesise} to produce a new
candidate.  Otherwise the optimised candidate solution is given back to
{\sc Verify Synthesis}.
...
%\item
\paragraph{6. {\sc Refine Abstraction}} changes $\phi_{static} = \phi_{static} \wedge \phi_{safety}^k$
to encompass any arising counterexample.  This is done by increasing the time horizon $k$ for the
...
\begin{figure}[t!]
{
 ...
 \begin{tikzpicture}[scale=0.3,->,>=stealth',shorten >=.2pt,auto, semithick, initial text=, ampersand replacement=\&,]
  ...
  \& complexnode/.pic={
    \coordinate (model);
    \node[minimum width=2.2cm, minimum height=0.8cm, fill=orange!20] (model2) at ([xshift=-3cm]model.center) {{\sc 4. Refine Abstraction}\\{\sc and Optimise}};
...
([xshift=5em]synth.north)
   node[align=left,yshift=-0.7cm,xshift=.5cm] {{$\phi$}\\{$\phi_{init}^K$}} %{$\phi_{safety}$}\\ 
