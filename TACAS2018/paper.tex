...
\begin{align}
\label{eq:ode1}
\dot{x}(t) = Ax(t)+ B u(t), \quad x \in \mathbb{R}^p, u \in \mathbb{R}^q, A \in \mathbb{R}^{p \times p}, B \in \mathbb{R}^{p \times q}, 
\end{align}
%
...
$\mat{A}_d$, $\mat{B}_d$, $\mat{C}_d$ and $\mat{D}_d$ denote the matrices that describe the discretized plant dynamics, whereas $\mat{A}$, $\mat{B}$, $\mat{C}$ and $\mat{D}$ denote the continuous plant dynamics.  In this work we will assume $\mat{D}_d=\mat{D}=\mat{0}$. Since $\vec{y}$ can be directly derived from $\vec{x}$, we will omit discussing it in the following unless strictly necessary.
