...
model.  (2) The second synthesis stage uses matrix eigen-decomposition 
and objective functions to iteratively optimize results computed by the first stage.
Whenever the optimization is unable to yield further results, it feeds back to the
synthesiser in a traditional CEGIS manner. This design yields a significant
speed-up compared to a traditional CEGIS scheme (i.e. without optimization).
...
duration we refer to as the sample time (cf. Appendix~\ref{sec:continuous_LTI}) into
\begin{align}
\label{eq:plant}
\left\{ 
\begin{array}{l}
...
but is rather inferred from its effect on the output $\vec{y}$ by using an observer.  
Essentially, a state observer is another dynamical model (below $\hat{\vec{x}}$) 
...
The feedback controller $\mat{K}$ is driven by the observer state $\hat{\vec{x}}$.
...
\resizebox{1.0\textwidth}{!}{
 ...
 \caption{Closed-loop digital control system with observer where the controller and the observer are implemented in a discrete domain\label{fig:observersystem2}}
\end{figure*}
 ...
 Consequently, we accelerate the dynamics of the system according to the model semantics in Sec.~\ref{sec:model_semantics} in order to compute the state $\vec{x}_T$  at a given time $T$. i.e. we find an equation for $\tau^n(\vec{x},\vec{u})$ given $\tau(\vec{x},\vec{u})$.
...
\end{array} \right ]
 ...
 \end{align}
$\lambda_i \in \mat{J}$ are the eigenvalues of $\mat{A}$, and $gm(\lambda_i)$ is the geometric multiplicity of $\lambda_i$.  $\vec{x}_T$ is the state of the system $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)$ at time t=T.
 \end{theorem}
 \begin{corollary}
 If $\mat{J}$ is diagonal and there exists a discrete dynamics matrix for a sampling time $T_d$ such that $A_d=e^{\mat{A} T_d}$, then $\vec{x}_k=A_d^k\vec{x} \text{ with } k \in \mathbb{R}^+$ is a witness of the system at time $t=kT_d$.
 \end{corollary}
\begin{proof}
...
\end{proof}
 ...
 inputs, described by $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)+\mat{B}\vec{u}(t)$, where $\forall t, \mat{B}\vec{u}(t)=\vec{u}(0)$. 
...
\end{proof}
 ...
 %+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 \subsection{Accelerated dynamics for models with discrete-time feedback}\label{sec:real_discrete_feedback_inputs}
 %+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
namely a new input value is produced only at every sample time.
%
\begin{theorem}
The expression
 \begin{align}
 ...
 $\mat{A}_d, \mat{B}_d$ describing the discretized plant dynamics is the state of the system
 \begin{align}
...
There are different types of stability, such as Bounded Input/Bounded Output (BIBO)
in which the requirement is that given a bounded input the output remains within
certain limits, or asymptothic stability in which the output is expected to
converge to a certain value. In our model, stability is directly related to
convergence, which means it directly relates to the eigenvalues of the dynamics. 
In this paper we use Jury's criterion~\ref{sec:closed_stability} to verify stability. 
...
\vec{y}_k=&\mat{C}_d\mat{T}_c\vec{x}_k'% + \mat{D}_{d}\vec{u}_k
...
We build upon this paradigm to create CEGORS (Counter Example Guided, Optimization
Refined Synthesis).  The main advantage of this approach, is that by optimizing the
synthesis results with respect to some objective function, we are able to reduce the
number of times the loop iterations, thus improving its performance.  Furthermore,
...
Our approach is illustrated in Figure~\ref{fig:CEGORS}. It relies on the
...
over-approximation errors and provides counterexamples for both
discrete and continuous dynamics to feed a CEGIS loop on the synthesiser
waiting for a new candidate solution. 

Our CEGORS loop is similar to CEGIS with solution generalization~\cite{david2017program}.
...
on which properties fail verification. In our case, the minimal abstraction
used by the synthesiser almost always ensures stability, so our objective function
focuses on safety, although we manage to do optimizations that
simultaneously target safety, stability and response time. This is done by
making the dynamics as convergent as possible while maintaining the
verification properties that exclude excesively convergent ones.
...
The CEGORS loop for this approach is described next.
...
described in Section~\ref{sec:FWL_effects}.
...
specification, it feeds back to the {\sc Optimise} phase which optimises
an objective functiion in order to minimise the magnitude of the error in the verification.
If the candidate is valid, we feed it to {\sc Discrete Time}. 

\item {\sc Optimise} 
% starts by taking $\mat{K},\mat{L} \not \models \phi_{static}$
% and synthesises $\mat{K}_{\langle C \rangle} = \mat{K}+\mat{\delta}_K, \mat{L}_{\langle C \rangle}=\mat{L}+\mat{\delta}_L$,
% where $\mat{K}_{\langle C \rangle}, \mat{L}_{\langle C \rangle} \models \phi_{static}$.  
% If $\mat{K},\mat{L} \models \phi_{static}$ but we have a counterexample for $\mat{K},\mat{L} \models \phi$,
% then we proceed to optimise the controller.
...
trajectories to go further).  In this case, we take the largest eigenvalue and
reduce it by a factor (smaller than .5) that still satisfies $\phi_{static}$.  The
resulting observer controller is then verified against the full specification and
the maximum deviation from the safe region is measured (maximum distance from
the discovered reach set to the safety region, measured perpendicularly to any
of the faces of the safe region). 
While this number is monotonically decreasing we continue to iterate until
it reaches a negative value (i.e. safe). If the number increases, we change
our objective function to the second stage.

In the second stage, at each iteration we take the complex eigenvalue
with the largest phase (largest angle in the complex space), and reduce its
imaginary component while maintaining the specification $\phi_{static}$.
The process ends when no imaginary part can be further reduced.
The resulting closed loop form converges faster than the original and may have a
smaller rotation, which facilitates meeting the safety specification (we note
that real convergent eigenvalues are trivially safe).

If we cannot synthesise a suitable candidate using optimisation, we signal the
{\sc Abstract} phase to refine $\phi_{synth}$ and request {\sc Synthesise} to
produce a new candidate.  Otherwise the refined candidate solution is given back
to {\sc Verify Synthesis}.
%
\item {\sc Discrete Time} verifies $\phi_{safety}^{\infty}$ (unbounded time)
...
\item {\sc Refine Abstraction} changes $\phi_{safety}$ to encompass any arising
...
thus creating hundreds of different benchmarks from this base.
...
%
\hline
\# & \multicolumn{1}{|c|}{Benchmark} & \multicolumn{1}{|c|}{order} & \multicolumn{1}{|c|}{count} & \multicolumn{2}{|c|}{CEGIS ($\mat{K}$)} & \multicolumn{5}{|c|}{CEGORS($\mat{K}+\mat{L}+T_s$)} \\
   &                                 & & & \multicolumn{1}{|c|}{-AR1} & \multicolumn{1}{|c|}{-OR} & \multicolumn{1}{|c|}{-AR2} & \multicolumn{1}{|c|}{-OR} & \multicolumn{1}{|c|}{Min} & \multicolumn{1}{|c|}{Median} & \multicolumn{1}{|c|}{Max}\\\hline
1  & DC Motor          & 2 & 10 & 9.5\,s & 2.1\,s& 1.9\,s& 3.4\,s& 1.2\,s &   3.1\,s &   14.1\,s\\
2  & Helicopter        & 3  & 10 & \xmark & 1.4\,s& \xmark & \xmark & 6.7\,s  &   18.9\,s & \xmark\\
3  & Inverted Pendulum & 4 & 10 &   9.7\,s &   0.6\,s& \xmark &112.6\,s& 12.0\,s  &  18.1\,s &  414.0\,s\\
4  & Magnetic Pointer  & 2  & 10 & \xmark & 44.1\,s& \xmark & \xmark& 4.6\,s  &  39.0\,s &  \xmark \\
%5 & Magnetic Suspension          & 2 & 1 & 12.2\,s& 0.6\,s & & & N/A  &   N/A &   N/A\\
5  & Pendulum          & 2 & 10 & 14.0\,s & 0.6\,s& 2.0\,s & 1.5\,s &1.2\,s  &   1.5\,s &   4.3\,s\\
6  & Suspension        & 4 & 10 & 73.7\,s& 48.3\,s & \xmark & 110.2\,s &24.0\,s  &   46.1\,s &  557.9\,s\\
7  & Tape Driver       & 3 & 10 & 10.1\,s& 12.4\,s& 140.3\,s &10.5\,s &2.6\,s  &   10.5\,s &   104.6\,s\\
...
Experimental results. All times are median (except for Min and Max). \xmark~indicates failure to find a controller (in the median case it means that more than half of the times we failed to find a controller), either due to timeout or insufficient numeric precision (bitvectors are limited to 64bits). -AR1 is standard CEGIS, -AR2 uses Solution Generalization but no refinement. -OR means Abstraction Refinement but no optimization refinement.}
\end{table}
...
%When the tighter restriction isdependent on the optimization parameters the overall speed is 

The smaller benchmarks (order=2) tend to show little or no improvement when we
apply CEGORS.  The reason is that for these benchmarks the synthesis time using
...
benchmarks that timed-out with AR1 (approx 20mins) are now running in under 2
minutes using AR2. This is the result of removing several multiplications which are
...
Since equation \eqref{eq:discretization} models \eqref{eq:dynamical}, we may use the semantics defined in Section
...
$\eta_1=\nu_1+\nu_1'+\nu_t$ and $\eta_2=\nu_2+\nu_2'$,
where $\nu_1$ and $\nu_2$ are the quantization noises introduced by the ADC and the DAC, $\nu_t$ is the noise introduced by the delay, and $\nu_1'$ and $\nu_2'$ are quantization noises caused by roundoffs in the FWL operations (see Section~\ref{sec:FWL_effects}).
