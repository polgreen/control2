...
<<<<<<< HEAD
from measurements of its inputs and outputs \note{(see Appendix~\ref{sec:observable})}. 
=======
from measurements of its inputs and outputs. 
>>>>>>> 47994b0211a56a3e76ec2e413851a0b687d47697
...
\end{proof}
 ...
 For such models, a state $\vec{x}_T$ is computed next.  \note{The next result is again known from literature, it's not a new theorem.}
\begin{theorem}
The state of the model $\dot{\vec{x}}(t)=\mat{A}\vec{x}(t)+\mat{B}\vec{u}(t)$ at time $T$ given a parametric input $\vec{u}$ is 
 ...
 \end{align}
<<<<<<< HEAD
 where $\forall t \leq T,\ \vec{u}(t)=\vec{u}$, and $\mat{A}_T$ as in \eqref{eq:continuous_tube_dyn2}\footnote{When $\mat{A}$ is not invertible, we use $\left[\begin{array}{cc}\mat{A}_T&\mat{B}_T\\ 0&\mat{I}\end{array}\right]=e^{\left[\begin{array}{cc}\mat{A}&\mat{B}\\0&0\end{array}\right]}$}.
=======
 where $\forall t \leq T,\ \vec{u}(t)=\vec{u}$, and $\mat{A}_T$ as in \eqref{eq:continuous_tube_dyn2}
 \footnote{when $\mat{A}$ is not invertible, we use $\left[\begin{array}{cc}\mat{A}_T&\mat{B}_T\\ 0&\mat{I}\end{array}\right]=e^{\left[\begin{array}{cc}\mat{A}&\mat{B}\\0&0\end{array}\right]T}$}.
>>>>>>> 47994b0211a56a3e76ec2e413851a0b687d47697
 \end{theorem}
 \begin{corollary}
...
=======
 and where $\mat{B}_T=\mat{A}^{-1}(\mat{I}-\mat{A}_T)\mat{B}$
  \footnote{as before, when $\mat{A}$ is not invertible, we use $\left[\begin{array}{cc}\mat{A}_T&\mat{B}_T\\ 0&\mat{I}\end{array}\right]=e^{\left[\begin{array}{cc}\mat{A}&\mat{B}\\0&0\end{array}\right]T}$}, 
>>>>>>> 47994b0211a56a3e76ec2e413851a0b687d47697
...
as classical in Control Theory,  %\footnote{More details on Jury's criterion is in Appendix~\ref{sec:closed_stability}.}, 
 ...
 on the characteristic polynomial of the model - a SAT solver will manipulate its coefficients. 
 \note{unclear really how SAT relates to manipulating real coefficients.... }
%
...
The benefit of this representation is that it has no multiplications, which are expensive to handle by SAT solvers. 
%
...
New problems arise due to FWL effects on these matrices. 
These are reflected in the canonical form of the resulting model,
...
\note{[next is unclear - how are matrices related to FWL errors? are they related to the quantities discussed in the previous section 5.2?] and $\mat{A}_{e}$ and $\mat{B}_{e}$ are the error caused by the FWL representation}.
...
be true.  We must therefore find a suitable solution to maintain this \note{[unclear:] separation} of polynomials.

Gershgorin disks~\cite{van1996matrix} allow us to limit the error
in the \note{[?]expected} eigenvalues.  Let $\delta_o=\max(\norm{{\mat{A}_e}_{i,*}}_1)$
...
true eigenvalues of the observer \note{clarify what matrix you're computing the eigenvalues of} and $\hat{\lambda}_{oi}$ the eigenvalues of
the perturbed observer \note{ditto}.  \note{unclear next, unless you're a control guy.} We note that in the case of stability we only care
about the error with respect to the \note{undefined: unit circle}, which is equivalent to
upper bounding the estimation of the largest eigenvalue.  Hence, given a
desired upper value $\overline{\lambda}$ \note{clarify 'desired' = how do you pick lambda bar?}, we apply Jury's criterion on the
polynomial \note{you need to relate the coefficients in the polynomial to the constants in the matrices discussed before.}
%
\begin{equation}
P_L'=\sum_{j=0}^p c_j\overline{\lambda'}^{p-j}s^j, \text{ where }  \overline{\lambda'}=(1+\delta_o)\overline{\lambda} \text{ and } c_j=l_j-a_j.
\end{equation}
%
In the case of the \note{[unclear - undefined] reachable eigenvalues}, the procedure is the same except
that we obtain our bound $\delta_c=\max(\norm{{\mat{A}_e}_{*,i}+{\mat{B}_e}_{*,i}}_1)$ \note{why? explain in close connection with formulae used.}
(\emph{i.e.} the largest column norm in $ \left[\begin{array}{c}\mat{A}_e\\ \mat{B}_e\end{array}\right]$) and
apply Jury's Criterion over
%
\begin{equation}
P_K'=\sum_{j=0}^p c_j\overline{\lambda'}^{p-j}s^j, \text{ where }  \overline{\lambda'}=(1+\delta_c)\overline{\lambda} \text{ and } c_j=k_j-a_j.
\end{equation}
...
may therefore find solutions that the synthesiser can't due to the use of limited precision.
...
3  & Inverted Pendulum & 4 & 100 &  277\,s& t.o. & 36& 0 & 20.0\,s  &  t.o. & 37 & 19\\
...
Experimental results. Minimum, and Median times for standard CEGIS vs CEGIS-OR on multiple benchmarks. No. indicates Number of benchmarks,
 t.o.~indicates timeouts (in the median case, it indicates more than half of the benchmarks timed out). Order is the number of states of the system.}
\end{table}

Using the described technique we managed to synthesize controllers for $509$ benchmarks (compared to $290$ found by our baseline), with a median synthesis time of $16.8$ seconds.
...
In those cases for which the system fails to find a controller, we use a
relatively short timeout, so it is possible that longer timeouts would instead lead to a
...
It is impossible at this stage for us to differentiate these from case where a controller does not exist.
...
%% when the initial coarse abstractions were sufficient to synthesise a
%% solution.
...
solutions that are not available using 64 bit plant precision (the maximum
allowed by the synthesiser in our implementation of standard CEGIS).
