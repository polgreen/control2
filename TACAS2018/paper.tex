...
Refined Synthesis) \note{not sure whether we need an acronym for this, we could just call it
CEGIS with optimization and abstraction refinement}.
Our approach is illustrated in Figure~\ref{fig:CEGORS} and consists of three main
phases:
...
waiting for a new candidate solution. \note{not sure about this sentence.
  Which is the CEGAR loop?}

The main advantage of this approach is two-fold:
\begin{itemize}
\item[1.] Optimizing the candidate solution in each
  iteration of the CEGIS loop by computing the local optimum
  enables us to obtain high quality counterexamples in the {\sc verify} phase.
  In turn, this has the potential to reduce the number of iteration of the CEGIS
  loop required to obtain a solution.
  \note{probably explain high quality counterexample}
\item[2.] We start with simpler (i.e. imprecise)
  abstractions that get
...
During the optimization, the minimal abstraction used by the synthesiser almost
always ensures stability, so our objective function focuses on safety,
although we manage to do optimizations that simultaneously target
safety, stability and response time.
\note{Are all the
  candidates computed by {\sc synthesize} stable? State clearly if
  so. You say ``almost always''.}
This is done by making the
dynamics as convergent as possible while maintaining the verification
properties that exclude excesively convergent ones.  
...
%\item
\paragraph{4. {\sc Optimise}} computes the local optimum in the neighborhood of
the current candidate solution. \note{is this really a local optimum?}
...
\note{Can we formally define the objective function?}

In this case, we take the largest eigenvalue and reduce it by a factor
(smaller than .5) that still satisfies $\phi_{static}$.  The resulting
observer controller is then verified against the full specification
\note{full specification?}
and the maximum deviation from the safe region is measured (i.e. maximum
distance from the discovered reach set to the safety region, measured
perpendicularly to any of the faces of the safe region).  While this
number is monotonically decreasing we continue to iterate until it
reaches a negative value (i.e. safe). If the number increases, we
change our objective function to the second stage.
\note{again, formal description? what is the objective function in this stage?}
