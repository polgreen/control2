...
We use $\mat{K}, \mat{L}$ because the synthesiser does not always meet the FWL specification. 
...
3  & Inverted Pendulum & 4 & 100 &  277\,s& t.o. & 32& 0 & 20.0\,s  &  t.o. & 37 & 19\\
...
valid solution. However, the solution is sound but incomplete, 
since we use approximations and over-approximations that may exclude
valid candidates in early stages, so it is impossible to tell if a solution exists in these cases. 
...
There are cases where newly found controllers are not due to time improvements,
but to the fact that the optimiser can reason about infinite precision
numbers and may therefore find solutions that are not available using 64 bit 
plant precision (the maximum allowed by our SAT solving tool).
