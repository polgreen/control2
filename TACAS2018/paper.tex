...
\footnote{Since we can only measure $\vec{y}$, the state-space $\vec{x}$ is not fully observable}.
...
\caption{Closed-loop digital control system with an observer, where the controller and the observer (bottom part) are implemented in a discrete domain,
  ...
  whereas the plant is continuous. \label{fig:observersystem}}
\end{figure*}
 %+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
where $\vec{x}'$ and $\vec{x}_e'$ are the state of the system and the error of the observer evaluated in the transformed domain, 
$\mat{A}_{c}$ the closed loop dynamics of the controller in canonical form, $\mat{A}_{o}$ the closed loop dynamics of the observer in canonical form, $\mat{B}_{o}$ the feedback gain, 
 $\mat{B}'$ the noise sensitivity, and $\mat{A}_{e}$ and $\mat{B}_{e}$ the error caused by the FWL representation.
