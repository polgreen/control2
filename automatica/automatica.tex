...
%-------------------------------
 ...
 The expression
 \begin{align}
 \vec{x}_k&=\vec{x}(t=k)=\mat{A}_k\vec{x}_0 : \dot{\vec{x}}=\mat{A}\vec{x} \wedge \mat{A}=\mat{S}\mat{J}\mat{S}^{-1}\\
 \mat{A}_k&= \mat{S}
 \left [ \begin{array}{cccc}
 e^{k\lambda_i}  & s_i\frac{k^{j-i}e^{k\lambda_i}}{(j-i)!} & \hdots  & s_i\frac{k^{p-i}e^{k\lambda_i}}{(p-i)!} \\
0 & e^{k\lambda_i}  & s_i\frac{k^{j-i}e^{k\lambda_i}}{(j-i)!} & \vdots \\
\vdots & & \ddots & \vdots \\
0 & \cdots & 0  &e^{k\lambda_i} \\
\end{array} \right ]
 \mat{S}^{-1}: s_i=\left\{\begin{array}{cc}1&gm(\lambda_i)>1\\0&gm(\lambda_i)=1\end{array}\right.
 \label{eq:continuous_tube_dyn}
 \end{align}
 where $\lambda_i \in \mat{J}$ are the eigenvalues of $\mat{A}$, and $gm(\lambda_i)$ is the geometric multiplicity of $\lambda_i$ is a witness of the system $\dot{\vec{x}}(t)=\mat{A}\vec{x}$ at time t=k.
 \end{theorem}
 \begin{corollary}
