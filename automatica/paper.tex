...
\note{throughout the article, we mix the notions of system, plant, and model. 
I propose to clarify throughout as follows: \\ 
we do not use the term plant (eliminate from text). \\ 
we have an underlying physical/real/actual system. \\ 
we have an analogue model of it (1), with continuous variables and continuous time. \\ 
we discretise time, obtain the reference model for this article (2).  \\ 
we define specs and properties over this discrete-time model. and we synthesise controllers $u_k$ (not $u(t)$) on it. 
}
...
\begin{itemize} 
\item
We account for the error introduced by finite precision arithmetic applied over the plant variables $x_k$ and $u_k$ \note{here I have replaced the continuous-time signals with discrete-time ones}, which are actually continuous quantities. 
...
which means that our verification \note{also called validation below ... } phase \note{I'm not sure that it's clear at this point what the verification phase consists of} must also account for the numerical error, 
as well as quantization errors caused by representing the physical plant at finite precision $\mathcal{F}_{\langle I_p,F_p \rangle}$. \note{Also, isn't this paragraph redundant after sec 3.7?}
...
Figure~\ref{fig:CEGIS}) and a validation phase (denoted by {\sc
  Verify}) \note{why do we use validation-VERIFY; this can be confusing, if you regard validation as a form of soft verification via data ... }, which interacts via a finite set of tests {\sc
...
This approach is sound \note{you mean complete?} if the current $k$ is greater than the completeness threshold (see later step).  
We also assume some finite precision for the plant and a given sampling time \note{sampling time comes out of nowhere - also not in the algorithm}.  
...
We present in this paper a selection of case studies for which we are able to synthesise controllers. In the full experimental environment, 
