...
This approach is sound, i.e., the controller produced is guaranteed to be safe, \note{you mean complete? No, i mean sound} if the current $k$ is greater than the completeness threshold (see later step).  
We also assume some finite precision for the plant and a given time discretisation, as described in section~\ref{sec:model}.  
...
Timeouts \note{above: failures} are indicated by~\xmark.  
