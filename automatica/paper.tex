...
In the multi-stage back-end we
\else
We
\fi
use bit-precise modelling in CBMC~\cite{ClarkeKL04}, a bounded model checker for C-programs, to accurately encompass this conversion. 
...
The values that the symbolically represented initial state may take are restricted by upper and lower bounds as described in section~\ref{}.
...
The multi-stage back-end uses 
\else
We use
\fi
the bit-precise modelling of finite-precision arithmetic in CBMC, 
...
and thus the precision is limited by the controller. This is a reasonable assumption based on commonly available hardware.
