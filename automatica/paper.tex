...
Digital Control; A/D converters; Control System Synthesis; Safety Analysis; Quantization Errors; Sampling.               % chosen from the IFAC 
...
restriceted dynamics.  We examine the case of Linear Time Invariant
(LTI) models, for which the synthesis of controllers is well
understood.  However, the use of digital control architectures adds
new challenges caused by artifacts specific to digital control, such
as the effects of finite-precision arithmetic, time discretization and
quantization noise introduced by A/D and D/A conversion.

In this paper we develop an automatic technique for generating digital
controllers for a given continuous plant model that are correct by
construction.  Specifically, we synthesize stable and safe,
...
stability, robustness, and safety, which define the behaviour
...
In this paper we present two novel approaches for the synthesis of digital controllers, 
both of which use a CounterExample-Guided Inductive Synthesis (CEGIS)~\cite{jha-icse10,
DBLP:conf/pldi/Solar-LezamaRBE05} as described next.
...
and makes the following contributions, where the fourth and he fifth denote
...
astrom1997computer}: %\note{Pls eliminate bold quantities, unless strictly needed. }
%
\begin{align}
\label{eq:ode1}
...
Equation~\eqref{eq:ode1} is soundly discretized in
...
The linear feedback controller is characterized by a gain matrix
$\mat{K}$ that influences the closed-loop dynamics
(Fig.~\ref{fig:digitalsystem}).  Later, we will address the issue of
variable quantization, as introduced by the ADC/DAC conversion blocks.
...
which will be key for abstraction-based synthesis.  
\note{This clashes with the model introduced above, with inputs - motivate why?}
Given a model of an iterative program with dynamics
\note{Pls eliminate unnecessary ``logical'' notation - we have discussed this already for the CAV18 submission, do we need to go through this again????}
%
\begin{equation}
\vec{x}_{k+1}=\mat{A}\vec{x}_k : \vec{x}_k \in \mathbb{R}^n \wedge \mat{A} \in \mathbb{R}^{n \times n} \wedge k \in \mathbb N_0,
\end{equation}
%
\note{material has been blindly pasted here - dimensions are all screwed up.}
...
%
\begin{equation}
\hat{X} = \{ \vec{x}_k : k \in \mathbb N_0 \wedge \vec{x}_0 \in X_0 \wedge \vec{x}_{k+1}=\mat{A}\vec{x}_k\}.
\end{equation}
%
It is easy to see that the infinite number of iterations would result in an
unbounded cumulative rounding error \note{why do we bring in the discussion on the rounding error here? why do we claim AA solves it? Is this the best manner to motivate the AA technique?}, but using Abstract Acceleration we
...
Introduce set $X=\{ \vec{x} \mid \mat{C}\vec{x} \leq \vec{d}\}$. 
\note{Why are we interested in this set? What does this set represent?}
...
The \textit{continuous stirred tank reactor} plant describes a linear model for PH dynamics of a PH 
