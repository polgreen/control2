...
that became very popular in the recent years.  CEGIS is an
...
and makes the following contributions, where the fourth and the fifth are
...
\item A further limitation of the work in~\cite{DBLP:conf/cav/AbateBCCDKKP17}
  ...
  numbers were employed for the representation of both the plant and the
  ...
  implementation with an SMT backend, allowing us to also use reals in order
...
the goal either to verify~\cite{daes20161} or to
...
in~\cite{picasso2003stabilization} introduces invariant sets as a mechanism to
...
\begin{itemize}
\item We represent the plant variables $x(t)$ and $u(t)$, which are real numbers, at a finite precision in our model, and so it is necessary to account for the errors introduced by this representation. We account for these errors in both back ends by using interval arithmetic, detailed in section~\ref{}. 
\item The quantisation errors by the ADC, which converts the analogue signal $x(t)$ into
the digital signal $x_k$ which is fed into the controller. We account for this error in 
the acceleration based back-end by introducing a noise signal $\nu_1$ that over-approximates this error, 
shown in the figure. Further details are found in section~\ref{}. 
In the naive back end we use bit-precise modelling in CBMC~\cite{} to accurately model this deterministic
conversion. In other words, CBMC constructs a boolean formula which explicitly unwinds the transition system for any symbolically represented input state, bounded by the initial conditions, and performs the ADC conversion precisely on that symbolically represented input state. When we verify safety, the boolean formula is satisfied only if there is an input state value for which the subsequent trace through the transition system eventually violates the safety property. 
\item The representation errors due to performing finite-precision arithmetic in the controller. 
In the acceleration based back-end, this is again treated by the introduction of noise that bounds the error.
The naive back end uses the bit-precise modelling of finite-precision arithmetic in CBMC, in the same way as we handled the ADC conversion. We assume that the controller
represents numbers to the same precision as the ADC.
\item We assume that the DAC converts numbers from the same precision as the controller to an analogue signal. 
As a result we can ignore any errors introduced by the DAC
\end{itemize}
 ...
 semithick, initial text=, ampersand replacement=\&,]
  ...
  \matrix[nodes={draw, fill=none, shape=rectangle, minimum height=.2cm, minimum width=.2cm, align=center
   ...
   ([yshift=2em]synth.east) edge node[align=center] {Candidate solution} ([yshift=2em]verif.west)
    ([yshift=-2em]verif.west) edge node[align=center] {Counterexample} ([yshift=-2em]synth.east)
...
In this section, we give a brief description of the CEGIS framework~\cite{jha-icse10,
  DBLP:conf/asplos/Solar-LezamaTBSS06}, as illustrated in Figure~\ref{fig:CEGIS}. 
...
exists.
%-------------------------------
\section{Formal specification of model properties} 
...
the problem statement is modified, by means of acceleration, into 
%
\begin{equation}
\hat{X} = \{ \vec{x}_k=\mat{A_d}^k\vec{x}_0 \mid k \in N_0 \text{ and } \vec{x}_0 \in X_0 \}, 
\end{equation}
%
and new model semantics and corresponding set are further defined as 
%
...
\cite{cattaruzza2015unbounded} extends this to LTI models with inputs, 
whereas we next extend the algorithm to continuous-time models, in view of the overall goal of this work. 
...
Our benchmark suite consists of 27 SISO models extracted from the 
...
feedback linearization~\cite{acrobot}.The \textit{antenna} benchmark 
describes the dynamics of the dynamics of the azimuth angle of an antenna 
motion system composed a gearbox, DC motor and amplifiers~\cite{gajic2008optimal}. 
The \textit{ballmaglev} plant corresponds to the discrete model of a simple 
...
The \textit{bioreactor} benchmark is a linear model identified by least squares 
for the cell mass concentration controlled through dilution rate of a 
bioreactor~\cite{bioreactor}.

The \textit{Chen\_ex1}, \textit{Chen\_ex2} and \textit{Chen\_ex3} benchmarks 
correspond to high-order control system discrete models generally used as 
case study for model order reduction techniques~\cite{CHEN1979389}. The benchmarks 
\textit{Cruise}~\cite{Franklin15} and \textit{CruiseHSCC}~\cite{Astrom08} are 
...
There are two continuous stirred tank reactor models named \textit{cstr} and 
\textit{cstrtmp} that describe the PH dynamics of a neutralization reaction 
...
flow stirred-tank reactor. The \textit{DC motor} plant describes the velocity 
dynamics of a basic brushed direct current electrical machine. 

The \textit{Flexbeam} benchmark is a plant employed in vibration studies; the 
physical  system consists of a flexible metallic structure with piezoceramic 
actuators and sensors. The \textit{guidance chen} benchmark is another 
...
\textit{helicopter} benchmark plant describes the dynamics of transitional motion 
with respect to a longitudinal plane and the rotational motion around the longitudinal 
axis for a coaxial helicopter.

The \textit{Invpendulum\_cartpos} and \textit{Invpendulum\_pendang} benchmarks
are models for the cart position and the pendulum angle of an inverted pendulum 
plant composed by a point mass must be equilibrated above the pivot point, which is 
mounted on an one degree of freedom cart that moves over a track through a DC motor.

The \textit{magpointer} corresponds to a magnetic pointer plant composed by a pointer 
employed in analogue gauges and indicators, whose angular position dynamics is 
controlled by a magnetic field. The \textit{magsuspension} describes the dynamic of
a magnetic car suspension system. The \textit{pendulum} plant consists of a swinging 
point mass suspended from a frictionless pivot by means of a negligible mass rod.

The \textit{Regulator} plant consists of a linear model of a synchronous electrical
machine connected to an  infinite bus~\cite{KOKOTOVIC198023}. The 
\textit{springer-mass damper system} plant is classic model for several mechanical 
...
The \textit{steam drum} benchmark describes the linear model for the level dynamics 
of a  boiler steam drum~\cite{boiler}. 

The \textit{suspension} benchmark corresponds to the model of a single wheel suspension 
system of a car, {\it i.e.}, the relative motion dynamics of a mass springer damp model, 
which connects the car to one of its wheels.  The \textit{tapedriver} benchmark
corresponds to the model of computer tape driver, \textit{i.e.}, a computer storage device 
...
2  & Antenna  & 6 &   &  &  &  & & & &\\
3  & Ballmaglev  & 3 &  &   &  &  & & & &\\
4  & Bioreact & 3 & 16,8 & 11200ms & 16,10 &14600ms & & & &\\
5  & Chen\_ex1& 3 & 16,8 & 7278ms  & 16,10 & 9196ms & & & &\\
6  & Chen\_ex2 & 8& & & 32,23 & 4683872ms &  & & &\\
7  & Chen\_ex3 & 7 &   & & & & & & &\\
8  & Chen\_ex4 & 9 &  & & & & & & &\\
9  & Cruise    & 1 & 16,8 & 6563ms & 16,10 & 6543ms &  & & &\\
10 & CruiseHSCC & 1 & 16,8 & 6587ms & 16,10 & 6567ms &  & & &\\
12 & Cstr & 1 & &  & & & & & &\\
13 & Cstrtmp  & 1 &   &  & & & & & &\\
14 & DCmotor   & 2 & 16,8  & 6879ms & 16,10 & 7786ms & & & &\\
15 & Flexbeam   & 6 &   &  & & & & & &\\
16 & Guidance chen  & 5 &   &  & & & & & &\\
17 & Helicopter   & 3 & 16,8 & 13728ms & 32,23 & 148012ms & & & &\\
18 & Invpendulum\_cartpos & 4 &   &  & & & & & &\\
19 & Invpendulum\_pendang & 2 & 24,12 & 9741ms & 16,10 & 28135ms & & & &\\
20 & Magpointer   & 2 & 16,8 & 10093ms & 16,10 & 20407ms & & & &\\
21 & Magsuspension  & 2 & 12,12  &22146ms  & 16,10 & 34808ms & & & &\\
22 & Pendulum   & 2 & 16,8 & 8503ms & 32,23 & 23987ms & & & &\\
23 & Regulator   & 5 &   &  & & & & & &\\
24 & Springmassdamper & 2 &   &  & & & & & &\\
25 & streamdrum   & 3 & 16,8  & 332422ms & 16,10 & 590501ms & & & &\\
26 & Suspension  & 4 & 16,8  & 17206ms & 16,10 &1372945ms & & & &\\
27 & Tapedriver   & 3 & 16,8  & 8188ms & & & & & &\\
\hline
%
\end{tabular}
\vspace{0.05in}
