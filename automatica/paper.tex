...
bit-precision.  We express these controller semantics as C programs, which is CBMC's preferred input language.  CBMC
symbolically executes every instruction in an input program by translating it to a boolean satisfiability problem.  At
the end of this process, the entire input program is translated to a boolean satisfiability formula which is satisfiable
iff a certain property about the program holds.  CBMC's API allows to specify these properties in the form of assertions
in the input program, and we use these properties to express that our generated controllers are modelled in a stable and
...
which we do using standard interval arithmetic. 
...
We account for this error in the acceleration-based back-end by introducing a noise signal $\nu_1$ that over-approximates it. 
Further details are found in Section~\ref{sssec:abstraction}.  
In the multi-stage back-end we use bit-precise modelling in CBMC~\cite{ClarkeKL04}, a bounded model checker for C-programs, to accurately encompass this conversion. 
In other words, CBMC constructs a Boolean formula that explicitly unwinds the transition system for symbolically represented initial states. 
\note{[rephrase:] The possible bounds that this symbolically represented initial state may take are restricted by upper and lower bounds as described in section . } 
The Boolean formula constructed by CBMC models the ADC conversions precisely on the symbolically represented states at every time step. 
When we check for a safety property, the Boolean formula is satisfied only if there is an input state value for which the subsequent trace through the transition system eventually violates the safety property. 
...
The multi-stage back-end uses the bit-precise modelling of finite-precision arithmetic in CBMC, 
in the same way as the ADC conversion was handled. 
We assume that the ADC represents numbers with a higher precision than the controller \note{why is this reasonable?}, 
and thus the precision is limited by the controller. 
...
represented at the fixed-point precision of the plant model.  \note{no assumptions on relative value btw controller and plant?}
...
\item The first {\sc verify} stage, {\sc safety}, shown in Alg.~\ref{alg:safetycheck} \note{$Num\_States = ?, plant\_typet = ? controller_typet =?$}, checks that the candidate
...
\note{This is only for spiralling dynamics - more generally, we need to clarify how the CT is computed. Shall we add the CAV17 proof?}
...
\note{ We represent these errors as non-deterministic
additive noise. }
...
13 & DCmotor   & 2 & 16,8  & 6.9\,s & 16,10 & 7.8\,s & 24,14 & 0.4\,s & 16,10 & 0.5\,s (4.0\,s) \\
