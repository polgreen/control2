...
\item We provide two novel algorithms: the first, a multi-staged approach, relies on 
  ...
  plant. 
\else  
\item We present a novel multi-staged approach to synthesising digital controllers, using 
...
and we use the Boost interval arithmetic library~\cite{DBLP:journals/tcs/BronnimannMP06} to bound the error in this representation. We use a compositional numeric library to perform the fixed point arithmetic for the controller~\cite{}. We check the controller is safe starting from each vertex state, which we have shown to be sufficient to prove safety from any initial state in the hypercube of possible initial states. 
...
The \textit{Chen\_ex1} and \textit{Chen\_ex2}
