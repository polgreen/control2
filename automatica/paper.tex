...
%\runtitle{Automated Synthesis of Safe Digital Controllers}  % Running title for regular 
...
Formal Verification \and Safety requirements \and Dynamical models \and Digital control synthesis \and Analog/Digital conversion \and Time sampling \and Quantization errors \\
...
The synthesis accounts for errors caused by the digitization effects introduced by the controller. 
The approach uses counterexample-guided inductive synthesis (CEGIS): 
an inductive generalization phase produces a controller that is known to stabilize the system but that may
...
a counterexample is provided to the inductive generalization and the process iterates until a safe controller is obtained.  
...
Specifically, we automatically synthesize %stable and 
safe,
...
%  \path       (q2.east) edge (zoh.west);
 ...
 the plant is intended to be an analogue model of the underlying real system, 
 whereas the controller is digital. 
...
\note{shall we add a note on time discretization errors? namely that we disregard them? } 
...
\note{[rephrase?] These errors are present in the real world [as opposed to being absent? vacuous statement?]}, and we must model them accurately in order to capture the full behaviour of the model:  
...
As a result we can ignore possible additional errors introduced by the DAC. 
...
We furthermore encompass the following error that is due to modelling (as opposed to the nature of the digital control).  
\begin{itemize} 
\item
...
and we use bounded model checking to precisely model the error introduced by the ADC/DAC and the finite-precision controller.
...
floating-point controllers are at a higher precision than the fixed-point ones. It may be that the precision of the fixed-point
...
than the fixed-point ones. There is one exception to this, which is due to the fact that it is not guaranteed that a controller
exists for all the time discretizations we give the algorithm and, in this case, the fixed-point algorithm might be forced to try more time discretizations
than the floating-point algorithm because a fixed-point controller lacks the precision to control the system under the first (larger) time discretizations. 
