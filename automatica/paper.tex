...
Later, we will address the issue of variable quantization, as introduced by the ADC/DAC conversion blocks.
...
where $K \in \mathbb{R}^{m \times n}$ is a state-feedback gain matrix, 
and $r_{k}$ is a reference signal (again digital). We will assume $r_k=0$, thus 
\begin{equation}
\vec{x}_{k+1}=(\mat{A}_d-\mat{B}_d\mat{K})\vec{x}_k
\end{equation}
Which corresponds to a closed loop system without inputs.
...
continuous time. For this reason we must first look at the accelerationm of continuous time dynamics.
