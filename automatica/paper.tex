...
\emph{The first approach} uses a multi-staged technique that starts by devising a
...
Additionally, this approach uses \emph{abstraction refinement} \cite{DBLP:conf/cav/ClarkeGJLV00},
...
time, in the current paper we present an approach for
...
\emph{safety}.  Our work indeed focuses on the \emph{safety} of internal states, which is usually
overlooked in the standard control literature with its default focus on stability properties.  
Moreover, our work integrates an abstraction/refinement (CEGAR) step inside the main CEGIS loop.

The approach underpinning tools such as Pessoa~\cite{mazo2010pessoa} and related extensions \cite{Anta2010,liu16} and applications \cite{zamani2014}, 
...
\note{Anticipate here connection with characteristic polynomial? See Section 4.}

%-------------------------------
\subsection{Errors}
\label{sec:errors}

We account for the presence of errors originating from a number of sources, shown in Figure~\ref{fig:digitalsystem}:
\begin{itemize}
...
we account for these errors in both approaches by using standard interval arithmetic. 
\item The quantisation errors by the ADC, which converts the analogue signal $x(t)$ into the digital signal $x_k$, 
which is then fed into the controller cf. Figure~\ref{fig:digitalsystem}. 
We account for this error in the acceleration-based back-end by introducing a noise signal $\nu_1$ that over-approximates it. Further details are found in section~\ref{sssec:abstraction}.  
In the multi-stage back-end we use bit-precise modelling in CBMC~\cite{ClarkeKL04}, a bounded model checker for C-programs, to accurately encompass this conver- sion. In other words, CBMC constructs a boolean formula that explicitly unwinds the transition system for symbolically represented inital states. The possi- ble bounds that this symbolically represented initial state may take are restricted by upper and lower bounds as described in section . The boolean formula constructed by CBMC models the ADC conversions precisely on the symbolically represented states at every time step. When we check for a safety prop- erty, the boolean formula is satisfied only if there is an input state value for which the subsequent trace through the transition system eventually violates the safety property.
...
We assume that the ADC represents numbers to the same precision as the controller, or higher, and thus the precision is limited by the controller. 
\item We assume that the DAC converts numbers to analogue signals with the same precision as the controller. 
As a result we can ignore any additional errors introduced by the DAC.  Both the assumptions about the ADC and the DAC are reasonable assumptions based on commonly available hardware. 
\end{itemize}

\subsection{Notation for fixed-point precision}  

In this paper we will use $\mathcal{F}_{\langle I,F \rangle}(x)$ to denote a real number
$x$ represented at a fixed-point precision using $I$ bits to represent the integer part of the number
and $F$ bits to represented the decimal part.

$\mathcal{F}_{\langle I_c,F_c \rangle}(x)$ to denote a real
...
%-------------------------------
 \subsubsection{Abstract Supports for Continuous-Time Dynamics}
...
which is achieved by selecting a number of points \note{on the boundary} of the set, 
...
and it leverages abstraction-refinement \cite{DBLP:conf/cav/ClarkeGJLV00} and acceleration (cf. Sec.~\ref{ssec:aa}) to improve scalability,  
...
This approach sound if the current $k$ is greater than the completeness threshold (see later step).  
...
\State assert($ \underline{u}  \leq u \leq \overline{u}$)
 \State set $x_0$ to be a \note{vertex state}, e.g., $[\underline{x_0},\underline{x_0}]$	
...
\item The first {\sc verify} stage, {\sc safety}, shown in Alg.~\ref{alg:safetycheck}, checks that the candidate
...
\note{This is only for spiralling dynamics - more generally, we need to clarify how the CT is computed. }
...
%  The multi-staged approach described in Sec.~\ref{sec:CEGIS-precision-incrementation}
...
%that form part of an uncountable space,
...
\note{but, this very shape of the characteristic polynomial holds only for LTI models in controllable canonical form.}
...
as further discussed in Section~\ref{sec:numeric_rep}. % More details on how to model
...
% safety specification $\phi_\mathit{safety}$.
  $K$ 
  that satisfies
  $\phi_\mathit{stability} \wedge \phi_\mathit{safety} \wedge \phi_\mathit{init}^{K}$ \note{you mean $\phi (K)$?} by invoking a SAT solver.  
  The safety specification in the SAT formula \note{[clarify:] is an over-approximation} that has constraints for a given set of iterations ($k$) \note{and initial values [conditions?]}.  
...
abstraction by adding the constraint $\mat{A}^k\vec{x}_0 \models \phi_\mathit{safety}$ \note{how is this local refinement practically performed?}.  
...
Recall we use $\mathcal{F}_{\langle I,F \rangle}(x)$ denote a real number $x$
...
%
$$\mathcal{F}_{\langle I_{c},F_{c} \rangle}(y(t)) = y_k :
...
\textcolor{blue}{Both approaches fail to synthesise a controller for half of the instances.}  
