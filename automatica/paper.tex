...
\note{[shouldn't this be a bullet point too?]}
...
\note{rephrase next - main clause has no verb:} Specifically, our model of the plant variables $x(t)$ and $u(t)$, which are, in reality, continuous quantities, with finite precision in our model. 
We \note{guarantee [?]} that the precision we use to represent the plant variables is at least as precise as the precision used in the digital controller and we use interval arithmetic to bound the errors, as detailed in Section~\ref{sec:numeric_rep}. 
...
Namely, as we are interested in capturing safety (to be discussed in Section~\ref{ssec:safespecification}\note{fix?}),  
...
The fixed-point arithmetic of the digital controller is performed on the upper and lower bound of each interval independently and the upper and lower bound of the result taken as the interval result\note{clarify what is meant by 'limit' and by 'each' (each amongst what?)}.  
...
\note{the first of the two equations is not clear - don't you miss a B matrix?}
%
\begin{align*}
