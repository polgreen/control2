...
such that the safety boundaries \rev{(assumed to be a compact set)} are not violated for any larger number of iterations.  
...
\begin{theorem}
  \label{th:safety}
\rev{Assuming that the initial state is in the safe set, }
...
$k{\geq}\overline{k}$ steps we have completed a full rotation, which results
in a point closer to the origin.  The synthesized $\overline{k}$ is the completeness threshold. 
\rev{[I will try to spin this more formally. ]}
\qed 
%
\end{proof}
