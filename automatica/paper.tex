...
In this paper, we examine the case of physical systems (plants) described mathematically as 
...
we perform automated control synthesis over a ``hybrid model'', 
...
\else
We present a novel approach for the synthesis of digital controllers making 
\fi
...
Our approach
\fi
uses a multi-staged \note{we should state what we mean by `stage' in multi-stage?} technique that starts by devising a
...
the number of iterations\note{stages?} required to sufficiently unwind the closed-loop model, 
such that the safety boundaries are not violated for any larger number of iterations.  
\note{we do not state the contribution about errors - this should be briefly mentioned here, before the item below. }
...
It is based on the abstraction of the model as a finite-state machine, 
...
\note{Add note explaining relationship of this work with previously published reviewed literature?} 
