...
DBLP:conf/asplos/Solar-LezamaTBSS06} as described next.
...
(Fig.~\ref{fig:digitalsystem}) such that
\begin{equation*}
\vec{u}_k=\mat{K}\vec{x}_k \Rightarrow \vec{x}_{k+1}=(\mat{A}_d-\mat{B}_d\mat{K})\vec{x}_k
\end{equation*}
Which corresponds to a closed loop system without inputs. Later, we will address the issue of
