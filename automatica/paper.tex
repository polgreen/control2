...
Our approach
\fi
uses a multi-staged technique\rev{, shown in Figure~\ref{fig:CEGIS-precision-increment}}, that starts by devising a
...
such that the safety boundaries \rev{(assumed to be a compact set)} are not violated for any 
larger number of iterations\rev{, illustrated in Figure~\ref{fig:ct}}.  
...
namely a quantitative safety requirement \rev{over a convex set around the origin}, 
...
meets a given safety specification \rev{that depends on a given set around the origin}.  
...
Furthermore, whilst the problem can handle general hyper-boxes, we will in practice work with set that contain the origin. }
...
Recall that this error is modelled precisely by bounded model checking.
