...
\note{In Figure 1, you're using a different formula than $\phi_\mathit{init}^{K}$, why? }
...
% safety specification $\phi_\mathit{safety}$.
  $K \in \mathbb{R}\langle I_c,F_c\rangle^n$ 
  \note{we have not introduced this notation, so this is not clear.}
  that satisfies
  $\phi_\mathit{stability} \wedge \phi_\mathit{safety} \wedge \phi_\mathit{init}^{K}$ by invoking a SAT solver. 
  \note{the corresponding block in the figure accepts two inputs - can you comment on them? also: is this SAT solving step obvious? Sat solvers resolve Boolean formulae, but here we have something else. So, explain block `Program search'. }
...
whereas the set $N$ is \note{non-deterministically chosen [obtained from above]}. 

 \note{Ref Abstraction Verifier in Figure?}
We next evaluate whether $\hat{X}^\# \models \phi_\mathit{safety}$ \note{how?}. 
If the verification holds we have a solution, and exit the loop \note{ref PASS?}.  Otherwise, we
find a counterexample iteration $k$ and corresponding initial point $x_0$
for which the property does not hold, which we use to \note{locally refine the
abstraction [how?]}.  When the abstraction cannot be further refined, we provide $(k, x_0)$ to the {\sc abstract} phase.
%
...
\begin{enumerate}
\item Find the constraints that invalidate the property as a set of counterexamples for the eigenvalues \note{sure, but how?}, which we define as $\phi_\Lambda$. 
...
the numerical error introduced by the finite precision used to represent the plant \note{(and its operations)},  
...
Then we account for the quantization errors introduced by the ADC\note{/DAC?}.
...
such that $I_p \geq I_c$ and $\allowbreak F_p \geq F_c$ \note{why is this reasonable?}).  
