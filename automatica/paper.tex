...
Then $U'\subseteq U_b' \oplus \vec{u}_c' \Rightarrow (\mat{I}-\mat{A}_T)U' \subseteq (\mat{I}-\mat{A}_T)U_b' \oplus (\mat{I}-\mat{A}_T)\vec{u}_c'$. Since $ (\mat{I}-\mat{A}_{bT})U_b'\supseteq  (\mat{I}-\mat{A}_T)U_b'$, then $(\mat{I}-\mat{A}_T)U' \subseteq (\mat{I}-\mat{A}_{bT})U_b' \oplus (\mat{I}-\mat{A}_T)\vec{u}_c'$ thus proving the theorem for $\vec{u}(t)=\vec{u}_0 : t \in [0\ T]$.
  
For the continuous time interval, let us look at the solution for the differential equation at time $T$ with a non-deterministic input $\vec{u}$.
\begin{equation*}
...
&=A_T\vec{x}_0+B_T\vec{u}_c+\int_{t = 0}^{T} \mat{S}e^{\mat{J} t} \mat{S}^{-1}\mat{B} (\vec{u}(t)-\vec{u}_c) dt\\
&=A_T\vec{x}_0+B_T\vec{u}_c+\mat{S}\int_{t = 0}^{T} e^{\mat{J} t} \vec{u}_{j}'(t) dt\\
&=A_T\vec{x}_0+B_T\vec{u}_c+\mat{S}\int_{t = 0}^{T} \sum_{k=0}^\infty \frac{1}{k!} \mat{J}^k t^k \vec{u}_{j}'(t) dt\\
&=A_T\vec{x}_0+B_T\vec{u}_c+\mat{S}\sum_{k=0}^\infty \frac{1}{k!} \int_{t = 0}^{T} t^k \mat{J}^k \vec{u}_{j}'(t) dt
\end{align*}
It follows from equation \eqref{eq:overinduced} that $\mat{J}^k \vec{u}_{j}'(t) \subseteq \mat{J}_b^k U_{bj}'$ (where $U_{bj}'=\mat{S}^{-1}U_b'$), hence
\begin{align*}
&X_T \subseteq A_TX_0+B_T\vec{u}_c+\mat{S}\sum_{k=0}^\infty \frac{1}{k!} \int_{t = 0}^{T} t^k \mat{J}_b^k U_{bj}' dt\\
\Rightarrow &X_T \subseteq A_TX_0+B_T\vec{u}_c+\mat{S} \int_{t = 0}^{T} \sum_{k=0}^\infty \frac{1}{k!}\mat{J}_b^k t^k U_{bj}' dt\\
\Rightarrow &X_T \subseteq A_TX_0+B_T\vec{u}_c+\int_{t = 0}^{T_s} e^{\mat{A}_b t} U_b' dt\\
\Rightarrow &X_T \subseteq A_TX_0+B_T\vec{u}_c+ \mat{A}^{-1}(\mat{I}-\mat{A}_{bT}) \mat{B} U_b
\end{align*}
Multiplying by $\mat{A}$ we get $X_T'$ as in \eqref{eq:contu_reachset}, thus proving the theorem.
\end{proof}
 ...
 %+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
The final case we analyse here is that of hybrid time systems, where the plant dynamics evolve in continuous time
...
The expression
 ...
 \vec{u}_t=\mat{K}\vec{x}_k  : kT_s \leq t < (k+1)T_s
 \end{align}
 \end{theorem}
...
\textcolor{red}{Dario: Do we still need this section? Where would the continuous time proof fit?}
...
Our benchmark suite consists of 18 MIMO models extracted from the 
control literature~\cite{acrobot,cstr,CHEN1979389,KOKOTOVIC198023,gajic2008optimal,Franklin15, maglev, converters, CTMS};
these models are discretized with different sample times, which thus leads to $167$ different benchmarks
to evaluate our proposal approach. 

The DC motor benchmark describes the velocity dynamics of a basic 
brushed direct current electrical machine. The automotive cruise control system 
benchmark is used to keep constant the speed of an automobile, 
tracking a desired speed reference, compensating disturbances and 
uncertainties related to relief and terrain.The pendulum benchmark consists of a swinging point mass suspended from 
a frictionless pivot by means of a negligible mass rod, while the inverted pendulum 
benchmark is a system similar to the pendulum, but the point 
...
The magnetic suspension benchmark corresponds to the discrete model of a 
...
The magnetic pointer benchmark is a pointer employed in analogue 
gauges and indicators, whose angular position dynamics is controlled 
by a magnetic field. The 1/4 car suspension benchmark corresponds to the model of a single 
...
The computer tape driver benchmark is a computer storage device used to read and write 
data on magnetic tapes. The acrobot benchmark corresponds to the model of a two-link 
underactuated robot manipulator, named acrobot, linearized by partial 
feedback linearization~\cite{acrobot}. The high-order system benchmarks correspond to a series of high-order control system model 
...
The continuous stirred tank reactor benchmark describes a linear model for PH dynamics of a PH 
neutralization process of an aqueous solution of sodium acetate with 
hydrochloric acid in a continuous flow stirred-tank reactor. The flexible beam benchmark is a plant 
usually employed in vibration studies; it consists of a flexible metallic structure with piezoceramic actuators and sensors.
...
The USCG cutter tampa heading angle benchmark describes the heading angle 
dynamics of a Coast Guard cutter. The voltage regulator benchmark~\cite{KOKOTOVIC198023} consists 
of a linear model of a synchronous machine connected to an  infinite bus.
...
The satellite attitude control system benchmark describes the dynamics of a satellite attitude, 
...
inertial frame during all the excursion. The springer-mass damper system benchmark 
is classic model for several mechanical systems dynamics. This benchmark describes a mass,
which is fixed to a node by means of a springer and moves under the action of a 
