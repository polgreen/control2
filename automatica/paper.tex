...
we account for these errors in both approaches \note{did we explain the two back ends already?. Yes. In the introduction.} by using interval arithmetic, as detailed in section~\ref{} \note{fix}. 
...
In the multi-stage back-end we use bit-precise modelling in CBMC~\cite{}, a bounded model checker for C-programs, to accurately encompass this conversion.   
In other words, CBMC constructs a boolean formula that explicitly unwinds the transition system for symbolically represented inital states. The 
possible bounds that this symbolically represented initial state may take are restricted by upper and lower bounds as described in section~\label{ssec:safespecification}.
...
We assume that the controller represents numbers at the same precision as the ADC. 
\item We assume that the DAC converts numbers to analogue signals with the same precision as the controller, or to a greater precision. 
As a result we can ignore any additional errors introduced by the DAC. This assumption and the previous assumption are reasonable
assumptions based on the availability and cost of popular hardware. 
\end{itemize}
