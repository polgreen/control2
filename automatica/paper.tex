...
later in this work, we will target a more complex objective,
quantitative safety requirements, which are not typical in the digital
control literature. We will precisely account for the digital nature
...
as a means for reducing the search space of possible safe controller. As discussed later, a safe controller is necessarily asymptotically stable, although the reverse is not true.
...
A discrete-time LTI system %as \eqref{eq:closedloopss} 
is asymptotically
...
one~\cite{astrom1997computer} (this simple sufficient condition can be generalized, 
...
specification gives rise to a requirement on the states of the model, such
...
instant, respectively.  This requires that the states will always be within an $n$-dimensional hyper-box.
...
\label{sec:BMC}
We use the bounded model checker CBMC~\cite{ClarkeKL04} to model controller behaviour on digital signals with finite
bit-precision.  
...
The key contribution of our work is that, when synthesizing controllers, we precisely model the finite-precision arithmetic in the digital controller, guaranteeing that these controllers implemented with finite-precision arithmetic are safe. The finite precision arithmetic introduces the following errors, shown in Figure~\ref{fig:digitalsystem}. These errors are present in the real world, and we must model them accurately 
...
As a result we can ignore any additional errors introduced by the DAC. Again, this assumption is reasonable based on commonly available hardware. It would, however, be straight forward to account for a DAC or ADC of different precision to the controller in our algorithm if necessary.
\end{itemize}
...
This is a synthesis problem, where the objective is to synthesise a valid $P$ that satisfies $\phi$ over all the $a$ terms. 
...
in order to narrow the search space of possible controllers, as detailed in Section~\ref{ssec:stabspecification}.

%-------------------------------
\subsection{Jury's stability criteria} 
\label{ssec:stabspecification}
%-------------------------------

There are a number of procedures that can be used for stability analysis of dynamical models\note{~\cite{daes20161, Bessa16} is self-citation really the best citation for this statement??}.
...
and the behaviour of the limited precision arithmetic used by the controller. 
...
As described in section~\ref{sec:BMC}, we use CBMC, a bit-precise bounded model checker to synthesise and verify candidate controllers. CBMC performs precisely the fixed or floating point arithmetic used in the controller, as well as the ADC/DAC conversions, according to the IEEE standards.
...
and we use the Boost interval arithmetic library~\cite{DBLP:journals/tcs/BronnimannMP06} to bound the error in this representation. We use a compositional numeric library to model the fixed point arithmetic for the controller~\cite{} within
double precision floating point numbers. We check the controller is safe starting from each vertex state, which we
will show to be sufficient to prove safety from any initial state in the hypercube of possible initial states. 
...
safety and stability properties of an LTI system
...
multiplication operation. This error is behaviour modelled by the bounded model checking.
...
\else
the instantiation of CEGIS we use to synthesise safe digital controllers, accounting for the precision issues detailed in previous sections. We use a multi-stage approach
that unwinds the dynamics up to a completeness threshold, modelling finite-precision arithmetic using bit-precise bounded model checking, and then verifying soundness of the resulting controller using interval arithmetic.
\fi
...
is the dimension of the state space, as before. 
\begin{proof}
...
Our benchmark suite consists of 20 SISO models extracted from the 
...
benchmarks correspond to high-order control system 
%discrete 
models employed as case studies for model-order reduction techniques~\cite{CHEN1979389}. 
The benchmarks \textit{Cruise}~\cite{Franklin15} and \textit{CruiseHSCC}~\cite{Astrom08} deal with 
...
tank reactor. The \textit{DC motor} plant  describes the velocity 
dynamics of a 
%basic brushed 
...
The \textit{suspension} models a single-wheel suspension system of a car, 
...
controllers is too low to be able to meet the safety specification. The floating point controllers take longer to synthesise
...
%1  & Acrobot     & 4 & 16,8 & 61529.8\,s &  & ~\xmark \\
...
The mean run-time for \textcolor{blue}{the successful} benchmarks is 152.6\,s for the fixed-point and 117.5\,s for floating-point implementations, respectively.  The mean run-time for \textcolor{blue}{the successful} benchmarks is 16\,s for the fixed-point and 11\,s for floating-point implementations, respectively.
...
The completeness threshold may be too large, thus causing a timeout, or controller simply may not exist for these benchmarks at this given time discretisation and controller
precision.
