...
\end{tikzpicture}
\caption{CEGIS with multi-staged verification for digital controller synthesis. We synthesise $K$}
...
\rev{ We assume full observability of the states of the model, namely the output states correspond with the model variables, which we can thus fully access.  }
...
We will assume $r_k=0$ \rev{(meaning that the reference signal is just a zero signal)}, thus obtaining the closed-loop model 
$\vec{x}_{k+1}=(\mat{A}_d-\mat{B}_d\mat{K})\vec{x}_k$ \rev{with the origin as its equilibrium point}. 
...
\rev{We refer the interested reader to~\cite{fadali} for further details about Jury's criterion, 
which we omit for the sake of space and since it is a standard result in the control literature that we just leverage for our overall approach. }
...
Recall that both these errors are modelled precisely by bounded model checking.
...
In conclusion, we can thus disregard these steady-state errors (caused by
finite-precision arithmetic) when stability is ensured by synthesis, and then verify  
...
unfolding the transition system (i.e., the closed-loop model) $k$ steps and by selecting a controller $K$ and a single initial state, 
such that the states at each step do not violate the safety requirement (see Section~\ref{sec:safety}).    
That is, we ask the bounded model checker~\cite{ClarkeKL04} if there exists a $K$ that is safe for at least 
one $x_0$ in our set of all possible initial states, and a given fixed-point precision for 
the controller, and where the input signal remains within the specified bounds. 
...
This approach is sound, i.e., the controller produced is guaranteed to be safe, if the 
current $k$ is greater than the completeness threshold (see later step).  
...
The first {\sc verify} stage, {\sc safety} is shown in Algorithm~\ref{alg:safetycheck}. 
...
if a controller is safe for each of the corner cases of the hyper-box of allowed initial states, 
i.e., the vertex states, then it is safe for any initial state in the hyper-box. 
\end{theorem}
\begin{proof}
Consider the set of initial states, $X_0$, which we note is convex since it is a hyper-box. 
...
%\todo{Add remark from reviewer that this number can be lower in actuality?} 
\rev{Whilst this full check on all vertices might not be necessary in general  
(it might be simplified in special cases or under special conditions), 
it does not represent the bottleneck of our overall method, so we can
safely rely on its current form. } 
...
In this second instance, consider the closed-loop matrix that updates the
states at each discrete time step, and select the eigenvalue $\vartheta$ 
...
Hence, $\overline{k}$ will again represent a global completeness threshold. }
\qed 
%
\end{proof}
...
correctly synthesized with respect to the effects of finite-precision arithmetic within a time-out set to 5 hours. 

The CEGIS engine is implemented as an integrated module within the C bounded
...
experiments.\footnote{\url{http://www.cprover.org/DSSynth/journal.tar.gz}}
%
...
Our benchmark suite consists of 19 case studies extracted from the control literature~\cite{acrobot,cstr,CHEN1979389,KOKOTOVIC198023,gajic2008optimal,Franklin15, maglev, converters, gajic2008optimal}. These case studies comprise control models, which we represent in state space form as in Equation \eqref{eq:ode1}. 
The models are time discretized, with sampling times ranging from 1s to 0.0001s. 
CEGIS initially starts with the coarsest discretisation (i.e., 1s sampling time), and if it fails to find a controller, it reduces the sampling time. 
The initial states are bounded between $0.5$ and $-0.5$ for all states, and the safety specification requires that the states remain between $-1.5$ and $1.5$ \rev{(as remarked above, the initial set lies within the safe set and contains the origin)}. The input bounds are selected individually for each benchmark.
...
where the control is used to maintain the speed of an automobile at a constant value (after tracking a desired speed reference), 
and compensating disturbances and uncertainties.  % related to relief and terrain. 
%There are two continuous stirred tank reactor models named 
The models \textit{cstr}  and \textit{cstrtmp}  describe the pH dynamics of a reaction 
...
describes a model for the cart position and for the angle of an inverted pendulum placed on the cart,  
...
which moves over a track by means of a DC motor.  
%
...
%
The \textit{magsuspension} describes  the dynamics of a magnetic car suspension system. 
...
orientation of a satellite with respect to an inertial frame. % during all the excursion. 
...
the evaluation study has the following overall experimental goal:  
\begin{enumerate}
\item[] 
...
\begin{table*}
\centering
\begin{tabular}{| l | c | c | c| r | c | r | c |}
%
\hline
% \multicolumn{1}{|c|}{Benchmark} & \multicolumn{1}{|c|}{Order} & \multicolumn{4}{|c|}{Run-time} \\
 Benchmark  &  Order  & $\mathcal{F}_{\langle I_p,F_p \rangle}$ & Time(s) & k & $\mathcal{F}_{\langle E_p, M_p \rangle}$ & Time(s)  & k\\\hline
Bioreact	&	2	&	8,8	&	15.35 & 4	&	10,6	&	23.76 & 2	\\
Chen	    &	3	&	8,8	& 	11.24& 0	&	10,6	&	14.25 & 0	\\
%Chen 2	    &	3	&	8,8	&	11.24	&	10,6	&	14.25	\\
...
Inverted %
pendulum	&	2	&	12,12 &	16.01&	2 &	10,6	&	18.73 &0	\\
Magnetic %
pointer	    &	3	&	12,12	&	1071.02	 & 10 &	10,6	&	207.60 & 9	\\
Magnetic %
suspension	&	3	&	20,20	&	56.9 & 2	&	10,6	&	998.3 & 6	\\
Pendulum	&	2	&	8,8	&	11.74 & 0	&	10,6	&	13.69 & 0	\\
Regulator	&	5	&		&	~\xmark	&  & 10,16	&	190.28	& 2 	\\
Satellite	&	2	&	8,8	&	13.91 & 3	&	10,6	&	16.92	& 7\\
Spring-%
mass-damper	&	2	&	12,12	&	16.09&	0  &	10,6	&	23.21 & 4	\\
Steam drum	&	3	&		&	~\xmark	&	& 10,16	&	21.16	& 4\\
...
We consider these times to be short enough to be of practical use to control engineers, and thus assert the success of 
%EG1. 
the overall objective of the study that we have raised above. \rev{The completeness threshold depends upon the controller
selected, and the SAT/SMT solver picks a controller with a low completeness threshold for all the time discretisations that we solve.
The completeness threshold is zero where the controller synthesised results in a system with real valued eigenvalues.} 
...
The provided experimental environment runs multiple discretisations for each benchmark, and lists the fastest as the resulting synthesis time. 
...
We have presented an automated approach to synthesize provably-correct digital state-feedback controllers that ensure 
%both stability and 
safety over state-space models.   
