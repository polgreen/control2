...
astrom1997computer}: \note{Pls eliminate vector quantities, unless strictly needed.}
%
\begin{align}
\label{eq:ode1}
...
linear combination of its states and inputs
\note{Do we need the output map? Where is it used?}
\footnote{Notice that since we
...
\note{Next needed?}
In this work we apply the standard assumption that
...
Introduce set $X=\{ \vec{x} : \mat{C}\vec{x} \leq \vec{d}\}$. \note{This notation clashes with that of the output map for the LTI model. }
...
\note{The second condition has already been defined above.}
%
\begin{equation}
...
%
\item Extract the vertices of the eigen-polyhedron $X_0'$ using the sound over-approximation of the double description algorithm \cite{fukuda1996double} from Section \ref{sec:dd} (line \ref{alg:getVertices}). 
\note{wrong refs.} 
It is worth noting that this algorithm uses a simplex to seed it: we will first discuss the simplex algorithm, which is also needed at the next step, and then the vertex enumeration algorithm used at this stage. 
%
...
simplex described in Section \ref{sec:simplex}.  \note{fix ref.} The objective functions are
...
$\mat{L}=\mat{L}_{\langle C \rangle}$ \note{Do we need this second matrix?}, we must be able to ensure that
