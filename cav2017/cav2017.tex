...
\vec{B}_d &= \int_{t = 0}^{T_s} e^{\vec{A} t} dt\ \vec{B} = \vec{A}^{-1} ( \vec{A}_d - \vec{I} ) \vec{B}\\
\vec{C}_d &= \vec{C}\\
\vec{D}_d &= \vec{D} \\
Q_d &= \int_{t = 0}^{T_s} e^{\vec{A} t} Q e^{\vec{A}^T t} dt
\end{align}
...
$$g(t) \rightarrow g(k) \Leftrightarrow G(S)=\mathcal{L}(g(t)) \rightarrow G(Z)=\mathcal{Z}(g(k))$$
where the poles $\hat{\lambda}_i=e^{-\lambda_iT} : \hat{\lambda}_i \in G(Z) \wedge \lambda_i \in G(S)$ are derived from \eqref{eq:discretize}.
