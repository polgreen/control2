...
\red{[Dario: please go through this section again, and clarify what sources of imprecision we consider where, distinguishing between plant and controller. also comment on why we operate on hat G rather than on tilde G - as we discussed yesterday. ]}
...
%
\begin{equation}
\hat{Y}(z)=\nu_{1}(z)+\hat{G}(z)C(z)R(z)+\hat{G}(z)\nu_{2}(z)-\hat{G}(z)\hat{C}(z)\hat{Y}(z). 
\end{equation}
%
...
%
\begin{equation*}
H_{1}(z)=\frac{1}{1+\hat{G}(z) \hat{C}(z)}, 
\end{equation*}
%
\begin{equation*}
H_{2}(z)=\frac{\hat{G}(z)}{1+\hat{G}(z) \hat{C}(z)}, 
\end{equation*}
%
\begin{equation*}
H_{3}(z)=\frac{\hat{G}(z) \hat{C}(z)}{1+\hat{G}(z) \hat{C}(z)}. 
\end{equation*}
...
\red{[wait I'm confused: to be precise, the next should be a tilde in view of the preceding comments on FWL!!] notice we are indeed approximating a C, so the equality comments from above do not apply! correct?}
%
\begin{equation}
...
\red{[hang in there - did we not consider FWL on $G(z)$? why not?]}
...
\blue{[so you mean we're not using Jury's criterion? pllease clarify.] the digital controller is designed with standard techniques for
control systems~\cite{Kuo:2002:ACS:579453, Ogata:1987:DCS:26170}. 
}
We start with an a-priori candidate solution with all coefficients zero: 
%
...
$$
\blue{[here recall that indeed this C equates to the tilde version, because \ldots ]}
