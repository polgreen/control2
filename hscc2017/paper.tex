...
\item We automatically generate {\em correct-by-construction} digital
  controllers using inductive synthesis.  Our application of program
...
\item Experimental results show that \tool is able to efficiently synthesize
  ...
  literature: the median runtime for our benchmark set is $197$\,s, i.e.,
  half of the controllers can be synthesized in less than five minutes.
...
digital controller).  To obtain a joint model, we fully discretize the
continuous plant.  We briefly recall the standard foundations of this step.

We only consider transfer function models, and require a $z$-domain transfer
...
Among the discretization methods in the literature~\cite{Franklin15}, the
zero-order hold (ZOH) discretization is suitable for representing the sample
and hold processes in complex systems~\cite{istepanian2012digital}.  The ZOH
...
is sufficient for stability studies over $G(s)$ \red{[we need to clarify the last sentence - why is it sufficient, where is this fact stated?]}.
...
the plant itself (\blue{all other errors described in this paper are ensured to
fulfil this property in our implementation}\red{[have we later argued that this is indeed the case?]}) 
\blue{and that enter the plant linearly}.

We employ a notation based on the polynomials coefficients to simplify 
...
\red{[confusing: we mix notations on abstract polynomials with those used for the plant above - the orders of the two are different ($N$ vs $N_G$) - please use the latter.]}
...
may have a different FWL than the controller),
%\red{[why is the DAC introducing a quantisation error again?]}
...
\red{[We have recast the model as in Figure 2, but never defined $\nu$ and never use it later - retain the two separate noise contributions as in Figure 1.]}
%, thus
...
\caption{Digital equivalent to the system in Figure~\ref{fig:sampledsystem} 
\label{fig:digsystem1}}
\end{figure}

%\red{[why is the output in Figure 2 tilde, whereas that in  figure 1 hat?]}
%
...
DAC conversions \red{[but the sample-and-hold happens only in the DAC, not in the ADC - right?]}\blue{[the sample-and-hold is on the ADC, the DAC is ZOH. I would just put `quantizations in the ADC and DAC conversions']}.  These sources are assumed to be non-deterministic, 
which poses no further restriction to our model, thus, the uncertain model
may be rewritten as a vector of coefficients in the z-domain using equations
(\ref{eq:coefficients}) and (\ref{eq:delta_coefficients})
%
\begin{equation}
...
order as the plant model, since tolerances of higher order would invalidate
feedback model (even tiny errors of higher order can move the poles by a
large amount preventing any single controller from stabilizing such a setup).
This is a reasonable assumption since most tolerances do not change the 
architecture of the plant, and models subject to such effects are
inadequate.
...
using this FWL with integer size $I$ and fractional size~$F$; $I$ affects
the range of the representation and is set to avoid overflows, while $F$
affects the precision and the truncation after arithmetic operations. 
\red{[we ought to align the following notations with those use in the CAV submission.]}
...
%------------------------------------
\subsection{Stability under Finite Representation and Quantisation Noise}
\label{sec:stability}
%------------------------------------
...
the following proposition gives necessary conditions for the closed-loop system 
BIBO stability in Figure~\ref{fig:sampledsystem}, with regards to the 
exogenous signals $R(z)$, $\nu_{1}$, and $\nu_{2}$, which are bounded.
...
($\nu_{1}$ and $\nu_{2}$) and FWL effects in the control software.  Note
...
noise is bounded.  The bound on the quantization noise is given by
...
%  with integer size $I_p$ and fractional
% size $F_p$, whose domain is $\mathbb{R}\langle I_p,F_p \rangle \supseteq \mathbb{R}\langle I,F \rangle$.
%
\begin{align}
\small
...
and $\Delta_b{G}$ represent the plant uncertainty caused by
...
The soundness of the model is ensured by the {\sc verify} phase. 
The {\sc synthesise} phase is only concerned with providing a candidate 
solution that is not ensured to work for any of the plant models. 
However, it is increasingly likely to be correct as the system loops 
through counterexamples. The overall result is that if a candidate solution 
is validated by the verifier, it is necessarily sound; otherwise, we may simply 
not find a valid solution in the given time. The {\sc verify} phase looks at 
two separate elements for soundness. The first corresponds to the controller fitness 
for all plant models. This phase ensures that there exists no counterexample plant 
that would create a closed-loop with unstable poles. The controller fixed word length 
is not relevant at this point since the parameters have already been chosen to fit 
this criteria (i.e., the candidate solution is FWL). The model is verified using 
the SAT solver backend in CBMC, which looks for the satisfiability of a plant 
with an unstable closed-loop. If no counterexample is found, then the controller 
is necessarily stable for all plants in the collection. The limitation of this approach 
is the data type used by the verifier to perform the evaluation. Since the actual plant 
works in the reals, whereas we use fixed/floating point arithmetic to evaluate it, 
we need to ensure that the rounding errors in the calculations do not cause the solution 
to be unsound. For this reason the second verification phase uses interval arithmetic 
with outward rounding to ensure that these errors do not exceed critical bounds that 
could make some (or all) plant(s) unstable. This means that whilst the solution may not be complete, 
it is sound for evaluating any collection of real plants with a defined FWL controller for all of them. 
The need for two verification phases is because whereas this second phase can ensure 
the stability of all plants in the collection, it is unable to provide a counterexample.
...
finite-precision arithmetic are considered (e.g., fixed-point arithmetic and
...
dynamics~\cite{Franklin15}, which require attitude (pose) control
...
The seventh set of benchmarks contains the discrete well-studied 
non-minimal phase model $G_{7}(z)$. Non-minimal phase model offers 
additional difficulties for the controller design~\cite{Doyle:1991:FCT:574259}.
%
...
%\end{equation}
%
The eighth set of benchmarks contains the \textit{Helicopter Longitudinal Motion} discrete model $G_{8}(z)$,
...
%\end{equation}
%
The ninth  set of benchmarks contains the \textit{Inverted Pendulum} discrete model $G_{9}(z)$, which 
...
%\end{equation}
%
The last set of benchmarks considers the discrete model $G_{12}(z)$
...
engine we apply a timeout of $8$ hours per benchmark, and $24$ hours for the approach 
...
refined with few counterexamples and increases of the fixed-point precision.  
\textcolor{red}{However, two benchmarks in our set required too many counterexamples 
to refine their solution (marked with $^*$ in Table~\ref{tab:results}).  
In these cases, the one-stage engine is able to complement the two-stage 
approach and synthesize a solution instead, albeit with significantly larger run-times.  
This performance difference is due to the fact that the one-stage verification engine does
...
The median run-time for our benchmark set is $48$\,s, implying that \tool
can synthesize half of the controllers in less than one minute.  Overall,
the average synthesis time amounts to approximately $42$ minutes.  We
...
one-stage engine for additional benchmarks, where the inductive
...
footnote.\footnote{\url{https://drive.google.com/file/d/0ByIexo3Z5N91QkhrMi1rdkZFTWc/view}}\footnote{CBMC version: \url{https://github.com/diffblue/cbmc/archive/7a6cec1dd0eb8843559591105235f1f2c4678801.zip}}
...
present approach is novel in the control system literature in that it puts
forward a fully automated synthesis approach that is algorithmically and
numerically sound.  In particular, \tool marks the first use of the CEGIS
paradigm, which handles plants with uncertain models and FWL effects over
the digital controller.  Note that implementing this architecture
efficiently requires adjusting the traditional CEGIS refinement loop to the
use of a two-stage verification engine: here the first stage performs fast,
but potentially unsound fixed-point operations, whereas the second stage
