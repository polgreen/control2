...
designed controller, which however broadly lack automation.  Recent work has
studied the stability of digital controllers considering implementation
aspects, i.e., fixed-point arithmetic and the word length~\cite{Bessa16}. 
They exploit advances in bit-accurate verification of C programs to obtain a
...
%
We further account for uncertainties on the plant models.  The resulting
closed-loop system is a program loop that operates on bit-vectors encoded
using fixed-point arithmetic with finite word length (FWL).  The three
effects of 1.~uncertaities, 2.~FWL representation and 3.~quantization
errors are incorporated into the model,
%
...
1. uncertainties in $G(s)$ (we denote the uncertain continuous plant by $\hat{G}(s)=\frac{G_n(s)+\Delta_p{G}_n(s)}{G_d(s)+\Delta_p{G}_d(s)}$ to explicitly encompass the effects of the uncertainty terms $\Delta_p{G}_{(\cdot)}(s)$) arising from tolerances/imprecision in the original model; 
...
and 
3. errors caused by quantization (which we model later as as external disturbances $\nu_1$ and $\nu_2$). 
...
We also remark that, due to its native digital implementation, 
there are no parametric errors ($\Delta_pC_n(z), \Delta_pC_d(z)$) entering in the controller. 
Thus $\hat{C}(z) \equiv C(z)$.
...
%   \& \coordinate (aux0);
 ...
 \caption{Closed-loop digital control system (cf. Section \ref{verifying-closed-loop-control-systems} for notations) \label{fig:sampledsystem}}
\end{figure*}
...
\aabatecmt{[wait I'm confused: to be precise, the next should be a tilde in view of the preceding comments on FWL!!] notice we are indeed approximating a C, so the equality comments from above do not apply! correct?}
\dariocmt{yes, I changed all C(z) below for tildes}
%
\begin{equation}
...
\aabatecmt{[so you mean we're not using Jury's criterion? pllease clarify.] 
%the digital controller is designed with standard techniques for 
%control systems~\cite{Kuo:2002:ACS:579453, Ogata:1987:DCS:26170}. 
}
\dariocmt{I'm just removing this. Not even sure what it means}
...
$$
\aabatecmt{[here recall that indeed this C equates to the tilde version, because \ldots ]}
\dariocmt{addressed above}
...
time (hence the noticeable digitazion effect).
...
\blue{[great, all good: how much uncertainty have you introduced? where precisely? in the coefficients or as an exogenous disturbance? \textcolor{green}{It is addressed in Sec \ref{experimental-setup}}]}
