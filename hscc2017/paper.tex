...
%------------------------------------
\subsection{Verifying Closed-loop Control Systems \\ \blue{Under the Effect of Imprecisions, Quantization, and Finite Word Length Representation}}
...
2. errors in the numerical calculations (equal to $0$ since we introduce them as external noises $\nu_1$ and $\nu_2$); 
and 3. roundoffs in the quantization (denoted $\Delta_b{G}_n(s),\Delta_b{G}_d(s)$).  
...
algorithms in our verification engine that can be used for stability
~\cite{daes20161, Bessa16}, one based on Schur's decomposition
...
us to simultaneously evaluate the whole collection of plants $\hat{G}(s)$
