...
\title{
Sound and Automated Synthesis of Digital\\
...
performance requirements are left to future work).
...
%\end{equation}
%
%\noindent We can obtain $G_{4}$ using the ZOH discretization defined
...
synthesizes word lengths for known control problems~\cite{jha-nsv13};
...
We have presented a method for synthesizing stable controllers, implemented
in a tool called \tool.  Our approach is it is fully automated and
algorithmically and numerically sound.  \tool marks the first use of the
CEGIS that handles plants with uncertain models and FWL effects over the
digital controller.  We transform the traditional CEGIS refinement loop into
a two-stage engine: here, the first stage performs fast, but potentially
...
%Finally, we will consider performance requirements.
