...
DAC conversions \red{[but the sample-and-hold happens only in the DAC, not in the ADC - right?]}.  These sources are assumed to be non-deterministic, 
...
a model that incorporates the effects of $\nu$ \red{[undefined, see above]} in $\vec{\hat{G}}$ to 
