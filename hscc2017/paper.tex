...
2. errors in the numerical calculations (which we model later as as external noises $\nu_1$ and $\nu_2$ \red{[no, these are the quantisation ones!]}); 
and 3. roundoffs in the quantization (denoted $\Delta_b{G}_n(s),\Delta_b{G}_d(s)$ \red{[these are related to FWL - we introduce them as such, but then use other operators in equation 10 - very confusing]}).  
...
%   \& \coordinate (aux0);
 ...
 \caption{Closed-loop digital control system \red{[has this hatted notation for the continuous-frequency plant ever been introduced?]} \label{fig:sampledsystem}}
\end{figure*}
...
%tolerances of higher order would invalidate the feedback model (even tiny 
...
%, and models subject to such effects are inadequate.
...
\end{equation}
%
\blue{[ next statement is potentially quite confusing, especially since you used the hatted variables next. ]}
In the case of a synthesized controller, because the synthesis is performed
using FWL, $\hat{C}(z) \equiv C(z)$.
\red{[discuss relationship with introduced terms $\Delta_b{G}_n(s),\Delta_b{G}_d(s)$?]}
