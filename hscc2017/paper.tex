...
\keywords{Control synthesis, CEGIS, finite word length, time sampling, quantization}
...
high degree of automation.  Modern synthesis engines combine automated
testing, genetic algorithms and SMT-based automated
reasoning~\cite{DBLP:conf/lpar/DavidKL15, DBLP:journals/corr/AlurFSS16a}.

By applying a state-of-the-art synthesis engine we are now able to present a
...
%\red{[have we later argued that this is indeed the case?]}) 
...
introduced by quantizations in the ADC and
DAC conversions.  These sources are assumed to be non-deterministic, 
which poses no further restriction to our model, thus, the uncertain model
may be rewritten as a vector of coefficients in the z-domain using equation
(\ref{eq:hatgvector})
%
\begin{equation}
...
uncertainties. Notice that the quantization noise is additive, which means
it does not affect the transfer function. Instead, we prove that the system
is stable given these non-deterministic noise inputs.
...
%
$$
\hat{G}(z) =\frac{\hat{G}_n(z)}{\hat{G}_d(z)}: \vec{\hat{G}}=\vec{G}+\Delta_p \vec{G}
\text{\quad(from~\ref{eq:coefficients},\ref{eq:delta_coefficients})}.
$$
%
...
S(z)=\mathcal{F}_{M_C}\langle I,F \rangle(C_n(z))\hat{G}_n(z)+\mathcal{F}_{N_C}\langle I,F \rangle(C_d(z))\hat{G}_d(z);
\end{equation}
...
that the model for quantization noise enters as a signal to be stabilized (i.e.,
...
\end{align}
%
we calculate $\tilde G(z)=\mathcal{F}_{M_G,N_G}\langle I_p,F_p \rangle(\hat{G}(z))$
%
\begin{align}
...
However, the benchmark \emph{SatelliteB2} with uncertainty ($G_{3b}$)
