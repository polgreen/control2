...
here, the Jury's method is chosen due to its efficiency.
...
i.e. if $\neg \phi_{stability}$ is satisfiable for $S(z)$, then the {\sc verify}
phase will provide values for each coefficients of $\hat{N_{G}(z)}$ and $\hat{D_{G}(z)}$,
...
\cdsay{Pascal, I also need the candidate solutions here}
