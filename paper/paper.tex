...
described model.  The first uses interval arithmetic 
\cdsay{Dario, can you please add a reference here?} to represent the
coefficients
$[{c}_i-\Delta_p{c}_i-\Delta_b{c}_i\ ,\
{c}_i+\Delta_p{c}_i-\Delta_b{c}_i+(2^{-F_b})]$ 
and the rounding operations outwards during the process. 
%\cdsay{dario, what coefficients are these?}
% This results in a
...
assume a certain precision for the uncertainty
followed by a second stage, where we check the soundness of our assumption.

Essentially, in the first stage, denoted by {\sc Uncertainty} in Fig.~\ref{fig:CEGIS}, 
assuming a certain precision \param{I_b}{F_b} 
