...
complete range of approximations, including time discretisation,
quantisation effects, and finite-precision arithmetic and their related
rounding errors.
%
...
Digital Systems Synthsis, named \tool.
%
%, which accepts the
...
Since the complexity of SAT solvers is worst case exponential with respect to the number of clauses (which are directly proportional to the size of the bit-vectors), our algorithm tries to solve the problem first for a low $I_b, F_b$ iteratively increasing the precision if it is insuficient.
...
Figure~\ref{fig:bode} shows the Bode diagram of the closed-loop system from~\cite{DBLP:conf/hybrid/WangGRJF16} 
(Fig.~\ref{fig:original}) and from our synthesiser (Fig.~\ref{fig:cegiscontroller}). 
The phase margin in Figure~\ref{fig:original} is negative, which means that the controller 
from~\cite{DBLP:conf/hybrid/WangGRJF16} is unstable, considering FWL effects. 
Note that the final solution from \tool is stable since it presents an infinite phase margin 
...
Figure~\ref{fig:step} shows the step responses of the closed-loop system with the original controller~\cite{DBLP:conf/hybrid/WangGRJF16} (Figure~\ref{fig:step0}), the first candidate solution (Figure~\ref{fig:step1}), and the final solution from \tool (Fig.~\ref{fig:step2}). The step response in Figure~\ref{fig:step0} confirms that the stability loss if we consider FWL effects. However, Figure~\ref{fig:step2} shows that the first candidate controller is able to stabilize the closed-loop system without uncertainties, but it is rejected by \tool since this solution is not considered to be sound. Finally, Figure~\ref{fig:step2} shows a stable behavior for the final (sound) solution, which presents a lower settling time.
...
and 24 hours for the approach using an one-stage engine.
...
respect to RQ2 we consider these times short enough to be of practical use
to control engineers.  Regarding RQ1 we further conclude that the approach
...
The experimental evaluation in section~\ref{experimental-results} shows
a very positive evaluation of \tool over a diverse set of real-world
benchmarks. Nevertheless, the set of benchmarks is limited within the
...
but this may not hold in general. Further experiments towards determining
