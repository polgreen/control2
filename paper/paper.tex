...
In particular, Figure~\ref{fig:bode} shows the Bode diagram of the illustrative example from~\cite{DBLP:conf/hybrid/WangGRJF16} 
...
with the original controller~\cite{DBLP:conf/hybrid/WangGRJF16} (Figure~\ref{fig:step0}), 
the first candidate solution (Figure~\ref{fig:step1}), and the final solution from \tool (Fig.~\ref{fig:step2}). 
The step response in Figure~\ref{fig:step0} confirms the stability loss if we consider FWL effects. 
However, Figure~\ref{fig:step2} shows that the first candidate controller is able to stabilize 
the closed-loop system without uncertainties, but it is rejected by \tool since this solution 
is not considered to be sound. Finally, Figure~\ref{fig:step2} shows a stable behavior 
for the final (sound) solution, which presents a lower settling time.
