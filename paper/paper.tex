...
Our application of program synthesis is not trivial 
...
%% in conjunction with a formal verification algorithm, for synthesising 
%% correct-by-design robust digital controllers for non-fragile closed-loop 
%% stability.

%% \item Experiments obtained with our synthesiser show that the combination 
%% of interval arithmetic to represent the controller coefficients and a two-stage 
%% verification approach to continuously refine precision, we are able to compute 
%% a parametric sound solution for $14$ out of $15$ digital stabilizing controllers 
%% given a set of continuous and discrete plant with parametric uncertainties.
...
%------------------------------------
\subsection{Sampling and Holding in Closed-loop Systems}
\label{ssec:SandH}
%------------------------------------
...
plant) and discrete parts ({\it e.g.}, digital controller).
...
The lemma \ref{eq:pulsetf} is ensures the poles and zeros matching with $\mathcal{Z}\left\lbrace{\mathcal{L}^{-1}\left\lbrace{\cdot}\right\rbrace_{t=kT}}\right\rbrace$ operations and includes the ZOH dynamics in the $(1-z^{-1})$ term, that is sufficient for the stability studies over $G(s)$
...
A typical digital control system system, with a continuous plant and a discrete controller,
...
stability, as shown in~\cite{Bessa16}.
...
%
Thus the perturbed controller model $\hat{C}(z)$ may be computed from the
