...
\item We designed and implemented an automatic approach for generating
  ...
  FWL. In particular, we found that a two-stage verification engine
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Automated Program Synthesis \\ for Digital Control}
...
The synthesis process is illustrated in Fig.~\ref{DSVerifier_process}. 
A~detailed description of DSVerifier tool can be found
in~\cite{IsmailBCFF15}.  Steps $1$ to $3$ are performed by the
...
\vspace{1ex}
\caption{Overview of the synthesis process\label{DSVerifier_process}}
\end{figure*}
...
an ANSI-C code input file (Steps 1-3), DSVerifier constructs a
...
Step $2$ and $3$ to calculate the parameters of the controller to be
synthesised (step C).  Finally DSVerifier builds an intermediate ANSI-C code
...
be satisfied if CEGIS synthesises controller coefficients satisfying $\phi$. 
...
that a synthesised solution is complete and sound with respect to stability
...
seconds) respectively, as follows
%
\begin{equation}
...
by the UNIX date command.  For the approach using a two-stage verification engine
we apply a timeout of 8 hours
per benchmark, and 24 hours for the approach using a one-stage engine.
