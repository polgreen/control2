...
The digital controllers synthesized in this paper are typically used in closed loops with continuous (and physical) plants. Thus, the proposed approach have to consider dynamics of continuous parts of system ({\it e.g.}, plant) and discrete parts ({\it e.g.}, digital controller), {\it i.e.} the synthesized controller have to consider the issues related to the hybrid systems studies.

For this purpose, the design procedure is based on a discretized model for a continuous plant. For transfer function models, we are interested in a $z$-domain transfer function that considers all the aspects about the continuous plant ($G(s)$) and the sampling and hold process.
...
Uncertainties in $G(z)$ may appear due to uncertainties in $G(s)$ (\emph{ie} $\Delta{G}_p(s)$), errors in the numeric calculations, or roundoffs in the quantization.
...
A typical hybrid system, with a continuous plant and discrete controller, is shown in Figure~\ref{fig:sampledsystem}, where the DAC and ADC converters introduce quantisation noises $\nu_{1}(z)$ and $\nu_{2}(z)$, $G(s)$ is the continuous-time plant model with parametric additive uncertainty $\Delta G_{P}(s)$ 
...
benchmark in Table~\ref{tab:results}. For most benchmarks the conjecture explained
in section~\ref{fixedpoint_verification} holds and the inductive back-end is able
to find a stable solution in less than one minute. This is possible if the inductive
solutions need to be refined with few counterexamples and fixed point precision
increases. However, some of our benchmarks did not meet these criteria. In these
cases the interval arithmetic back-end was able to complement the inductive approach
...
SatelliteC2            & 369\,s   & \tbmark & 76\,s   & \tbmark\\
SpringMassDamper       &          &         &         & 
\end{tabular}

\vspace{0.5em}
\scriptsize{* = solved with interval back-end}
\vspace{0.5em}
...
for synthesising stable controllers in the majority of the benchmarks, and can be
