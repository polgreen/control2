...
control systems such as handling the effects of quantisers 
and FWL. 

\item We provide experimental results showing that
our fully automatic CEGIS-based approach is able to synthesise 
stable controllers for a set of intricate benchmarks taken from 
...
\begin{equation}
\label{eq:pulsetf}
G(z,T) = \mathcal{ZOH}\left\lbrace{G(s)}\right\rbrace =(1-z^{-1})\mathcal{Z}\left\lbrace{\mathcal{L}^{-1}\left\lbrace{\frac{G(s)}{s}}\right\rbrace_{t=kT}}\right\rbrace.
\end{equation}
%
...
system have to be considered.  Considering that the quantizer $Q1$ is the
...
$Q2$) are uncorrelated white noises and their amplitudes are always bounded by the half of quantization step, {\it i.e.} $\vert \nu_{1} \vert \leq \frac{q_{1}}{2}$ and $\vert \nu_{2} \vert \leq \frac{q_{2}}{2}$, where $q_{1}$ and $q_{2}$ are respectively the quantization step of  ADC and DAC.
% 
\end{myassumption}
...
the unit circle.  \textcolor{red}{Analyzing Eq.~\eqref{eq:outputfunctions}, the following lemma
...
assumption~\ref{whitenoise} and the Lemmas~\ref{lemma:variance}
and~\ref{lemma:boundednoise} are true.
