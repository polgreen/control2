...
\end{equation}
%
we have $\vec{\hat{G}}=\vec{G}+\Delta{\vec{G}}$ \red{[uh, now the TF are vectors - confusing]}.  
In the following we will either represent the functions $G(z)$, $C(z)$, 
or work over their respective coefficients in vector form.   
...
uncertainty $\Delta_p \vec{G}_s$, $R(z)$ is the reference signal, $U(z)$ is
...
lemma states necessary conditions for BIBO stability of closed-loop system
in Figure~\ref{fig:sampledsystem}, since the the exogenous signals $R(z)$,
$\nu_{1}$, and $\nu_{2}$ are bounded.
...
is complete and sound with respect to stability property~$\phi$, since it
...
the representation of these coefficients.
...
scripts to reproduce the experiment, all benchmarks and the CEGIS tool is
provided in the
