...
the unit circle.  Analyzing Eq.~\eqref{eq:outputfunctions}, the following
lemma states necessary conditions for BIBO stability of the closed-loop system
...
%
A feedback hybrid control system as given in Figure~\ref{fig:sampledsystem},
...
\vec{G} \Rightarrow \hat{G}(z)=\frac{\hat{N}_{G}(z)}{\hat{D}_{G}(z)}$, is
...
the domain $\mathbb{R}\param{I_b}{F_b}$ for the plant's coefficients.  Note
that $I$ and $F$, as well as $I_b$ and $F_b$, denote the integer and
fractional size, respectively, with $\mathbb{R}\langle I_b,F_b \rangle
...
scripts to reproduce the experiment, all benchmarks and the DDSynth tool is
provided in the
