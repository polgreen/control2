...
output sampled by a ADC and the digital input provided by ZOH.  It is equivalent at the sample times ({\it i.e.}, $t=kT$) to the continuous transfer
...
The controller and the plant are linear time-invariant (LTI) systems, {\it i.e.}, any source of
nonlinearity ({\it e.g.}, saturation, overflow, ADC/DAC nonlinearities) will
be disregarded. Additionally, the pulse transfer function obtained via \eqref{eq:pulsetf} using an appropriate sample rate ({\it i.e.}, obeying the Nyquist sampling conditions)  may be considered a sufficient discrete-time representation of $G(s)$.
%
\end{myassumption}

Although the pulse transfer function $G(z)$ represents adequately $G(s)$ in z-domain, the model $G(z)$ may not be accurate and some uncertainties may not be covered by that model. These uncertainties may be parametrically expressed by an additive transfer function $\Delta G(z)$, resulting in a uncertain model $\hat{G}(z)$, such that:
\begin{equation}
...
A typical hybrid system is shown in Figure~\ref{fig:sampledsystem}, where the DAC and ADC converters introduce quantisation noises $\nu_{1}(z)$ and $\nu_{2}(z)$, $G(s)$ is the continuous-time plant model with parametric additive uncertainty $\Delta G_{P}(s)$, $R(z)$ is the reference signal, $U(z)$ is the control signal, and $\hat{Y}(z)$ is the output signal affected by the noises and uncertainties in the closed-loop system.
...
%size $F$, we shall denote this domain as $\mathbb{R}\langle I,F \rangle$ 
and assume that there exists a function $FWL[\cdot]:\mathbb{R}^{n}\rightarrow \mathbb{R}^{n}\langle I,F \rangle$, which applies the FWL effects to a polynomial. Thus the perturbed controller model $\hat{C}(z)$ may be computed from the original model $C(z)=\frac{N_{C}(z)}{D_{C}(z)}$ using the following expression:
\begin{equation}
...
In order to obtain a sound synthesis of digital controller, the effect of FWL implementation in controller and quantisation noises in closed loop system have to be considered. Considering that the quantizer $Q1$ is the source of a white noise with zero mean $\nu_{1}$ and $Q2$ is the source of a white noise with zero mean $\nu_{2}$, the following equation may written for the system in figure~\ref{fig:sampledsystem} considering the parametric uncertainties $\Delta G(z)$ and the FWL effects on controller  ($\hat{C}(z)$):
\begin{equation}
...
\item the controller $\hat{C}(z)$ is a minimum-phase system. 
\end{itemize}
\end{mylemma}

The Lemma~\ref{lemma:eq_int_stab} provides the necessary conditions for synthesis of digital controllers able to stabilize closed loop system, considering plant parametric uncertainties ($\Delta G_{P}(z)$), quantisation noises ($\nu_{1}$ and $\nu_{2}$) and FWL effects in digital controllers. Based on it, the model for plant uncertainty due quantisation noise ($\Delta G(z)$ may be disregarded if the conditions on Lemma~\ref{lemma:eq_int_stab} are attended and if the quantisation noise is bounded. 

The quantisation noise is indeed bounded, since assumption~\ref{whitenoise} and the Lemmas~\ref{lemma:variance} and~\ref{lemma:boundednoise} are true.
...
In particular, \cite{widrow1956} models the quantisation noise as a gaussian
...
The z-transform of the quantisation noise $\nu(z)=\mathcal{Z}\left\lbrace\nu(k)\right\rbrace$ in figure~\ref{fig:digsystem1} may be calculated through the power spectral density of $\nu(k)$, thus:
\begin{equation}
\nu(z)= \sigma_{Q}^{2}.
\end{equation}
\end{mylemma}
