...
tasks; we focus on automated correct-by-design synthesis algorithms, as
...
end, we propose a CEGIS-based solution that uses inductive invariant
synthesis in conjunction with a digital verification algorithm to find a
parametric solution for a digital controller given a continuous plant
specification.
%
\end{abstract}
...
with fixed word length.  \blue{The deterministic effect of the FWL is considered
and quantization errors are included in the model as a additive uncertainty 
...
output sampled by an ADC and the digital input provided by the ZOH.  It is equivalent at the sample times ({\it i.e.}, $t=kT$) to the continuous transfer
function $G(s)$ driven by ZOH.  
\red{[what does it mean that it is continuous ``at the sample times''? We're working in frequency. Are you matching poles? There is still no clear relationship between continuous and samples transfer functions, it is not formal in my opinion.]}
This pulse transfer function can be computed as
%
\begin{equation}
\label{eq:pulsetf}
...
we will use the notation $G(z)$ to represent the pulse transfer function.
%
\end{mylemma}
...
signal $\tilde u(k)$ \red{[is this a new notation over the time domain? It does not reflect the signals used in the plot -- utterly confusing!]} at instant $k$ and produces a rectangular pulse of height
...
\begin{myassumption}
%
\label{assumption:cont_and_plant}
The controller and the plant are linear time-invariant (LTI) systems, {\it i.e.}, 
\red{[!! of course, we're working with transfer functions!!]}
any source of
nonlinearity ({\it e.g.}, saturation, overflow, ADC/DAC nonlinearities) will
be disregarded \red{[how is this related to ADC/DAC? very confusing.]}
. Additionally, the pulse transfer function is obtained via \eqref{eq:pulsetf} using an appropriate sample rate ({\it i.e.}, obeying the Nyquist sampling conditions)  is a precise discrete-time representation of $G(s)$. \red{[clarify this: this cannot be an assumption, since it is hinging on a specific numerical scheme. Assumptions deal with structural properties for example, or choices of certain parameters ... ]}
%
\end{myassumption}

Although the pulse transfer function $G(z)$ represents adequately \red{[clarify adverb]} $G(s)$ in the z-domain, 
the model $G(z)$ may not be accurate and some uncertainties may not be covered by that model. 
These uncertainties may be \red{[clarify use of conditional tense]} parametrically expressed by an additive transfer function $\Delta G(z)$, resulting in a uncertain model $\hat{G}(z)$, such that:
\begin{equation}
\hat{G}(z)=G(z)+\Delta G(z),
\end{equation}
that may be \red{[\ldots]} represented by the following transfer function:
\begin{equation}
...
\end{bmatrix}
\end{equation}
\red{[where is this new variable used later?]}


A typical hybrid system \red{[clarify the adjective ``hybrid'']} is shown in Figure~\ref{fig:sampledsystem}, where the DAC and ADC converters introduce quantisation noises $\nu_{1}(z)$ and $\nu_{2}(z)$, $G(s)$ is the continuous-time plant model with parametric additive uncertainty $\Delta G_{P}(s)$ 
...
%   \& \coordinate (aux0);
   ...
   \& \node[rectangle,draw,
	minimum width=1cm,
	minimum height=1cm,
        label=\textbf{Plant}] (gs) {{\sc $G(s) + \Delta G(s)$}};
...
%size $F$, we shall denote this domain as $\mathbb{R}\langle I,F \rangle$ 
and assume that there exists a function $FWL[\cdot]:\mathbb{R}^{n}\rightarrow \mathbb{R}^{n}\langle I,F \rangle$, which applies the FWL effects to a polynomial \red{[explain index n! why do you need to work on a Euclidean domain?]}. Thus the perturbed controller model $\hat{C}(z)$ may be computed from the original model $C(z)=\frac{N_{C}(z)}{D_{C}(z)}$ using the following expression:
\begin{equation}
...
A feedback hybrid control system as shown in Fig.~\ref{fig:sampledsystem}, with a FWL implementation of the digital controller $\hat{C}(z) = \frac{FWL[N_{C}(z)]}{FWL[D_{C}(z)]}$ and uncertain discrete model of the plant $\hat{G}(z)=G(z)+\Delta G_{P}(z)=\frac{\hat{N}_{G}(z)}{\hat{D}_{G}(z)}$, is BIBO-stable if and only if: 
\red{[this statement does not seem to encompass the quantisation effects on the plant?]} 
\begin{itemize}
...
The Lemma~\ref{lemma:eq_int_stab} provides the necessary conditions for synthesis of digital controllers able to stabilize closed loop system, considering plant parametric uncertainties ({\it i.e.}, $\Delta G_{P}(z)$), quantisation noises ($\nu_{1}$ and $\nu_{2}$) and FWL effects in digital controllers. Based on it, the model for plant uncertainty due quantisation noise ({\it i.e.}, $\Delta G_{Q}(z)$) may be disregarded if the conditions on Lemma~\ref{lemma:eq_int_stab} are attended and if the quantisation noise is bounded. 

The quantisation noise is indeed bounded, since the following assumption~\ref{whitenoise} and the Lemmas~\ref{lemma:variance} and~\ref{lemma:boundednoise} are true.

\red{[first: lemmae precede theorems. \\
second: bounded is not energy bounded. ]}
...
By definition~\cite{poularikas2000transforms}, the power spectral density of  $nu(k)$ is the z-transform of $R_{\nu\nu}(\tau)$, thus
\begin{equation}
...
\red{[relate this discussion with the FWL one (and related notations) above.]}
...
Two approaches have been taken to synthesise a stabe controller based on this model.
The first uses a nondeterministic plant selected from $\mathfrak{G}(z)$ where the expected error of the bit vector calculations is upper bound at the source by selecting a conservative $\Delta{G}_b(z)$ that encompasses \red{[how? is this always possible?]} its propagation over a number of operations. The verification phase produces a counterexample if some instance of $\mathfrak{G}(z)$ does not verify the properties and the CEGIS loop iteratively looks for a new controller that verifies the supplied counterexamples.
Although it is fast, the conservative error calculation can be restrictive to a small number of benchmarks.

The second approch uses interval arithmetic, representing the coefficients $[{c}_i-\Delta_p{c}_i-\Delta_b{c}_i\ ,\ {c}_i+\Delta_p{c}_i-\Delta_b{c}_i+(2^{-F_b})]$ 
...
We use this method to check the stability in the $z$-domain for the characteristic polynomial $S(z)$ defined in~\eqref{eq:internal_stab_lemma}.
...
\resizebox{.7\textwidth}{!}{
 \begin{tikzpicture}[scale=0.3,->,>=stealth',shorten >=.2pt,auto, semithick, initial text=, ampersand replacement=\&,]
  \matrix[nodes={draw, fill=none, shape=rectangle, minimum height=.2cm, minimum width=.2cm, align=center
   ...
   \path
    ...
    ([yshift=-2em]verif.west) edge node {Counterexample} ([yshift=-2em]synth.east)
 ...
 \caption{Counterexample-Guided Inductive Synthesis of Closed-loop System (CEGIS paradigm). \label{fig:CEGIS}}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
