...
(Step A), and a transfer function $f$ represented by Eq.~\eqref{FWL_tf} 
(step B) to calulate the implementation $f(\hat{C}(z)) \rightarrow \tilde C(z)$ 
of the controller (step C).  Finally DSVerifier builds an intermediate ANSI-C code
...
domain.
A second remark is that the synthesis and verification engines use fixed/floating point arithmetic,
therefore we also need to examine the effect of discretization during these operations.
