...
classically studied in the digital control literature.
%
In this study, we plan to provide a principled approach to extend this
literature in a number of directions: 
%we are interested in quantitative tasks; 
we focus on automated correct-by-construction synthesis algorithms, as
opposed to verification (or validation) objectives over a designed architecture; 
and we encompass the complete range of
...
length (FWL). 
\red{[acronym later defined as finite-word length - please unify use]} 
The deterministic effects of the FWL and
...
\item We design and implement an automatic approach for generating
  ...
  in most cases, to a speed-up of at least two orders of magnitude over a
...
(\emph{i.e.} $\Delta_p{G}_n(s),\Delta_p{G}_d(s)$ \red{, as defined later?}), errors in the numeric
calculations, or roundoffs in the quantisation.  These uncertainties \red{may be [can be?, are?]} 
...
the plant itself (\red{other errors [which precisely? all others?]} are ensured to fulfil this property in our
implementation).
...
\red{[I've tried to clean the notations above. BUT: you cannot later add a transfer function with a vector of coefficients! Heresy!]}
...
A typical digital control system system with continuous plant and discrete
controller is illustrated in Figure~\ref{fig:sampledsystem}.  The DAC and
ADC converters introduce quantisation noises $\nu_{1}(z)$ and $\nu_{2}(z)$ \red{[this is an assumption and as such must be stated.]},
$G(s)$ is the continuous-time plant model with parametric additive
uncertainty $\Delta_p \vec{G}_s$ \red{[above you used n, d subscripts, here s?]}, $R(z)$ is the reference signal, $U(z)$ is
...
%   \& \coordinate (aux0);
   ...
   \& \node[rectangle,draw,
	minimum width=1cm,
	minimum height=1cm,
        label=\textbf{Plant}] (gs) {{\sc $G(s) + \Delta_p G_s$}};
...
%represent both quantisation noises ($\nu_{1}$ and $\nu_{2}$) what can be
...
%\end{equation}
%%
%where $H_{1}R(z)$ is exactly equal to $Y(z)$ without quantisation noise effects, thus:
%%
%\begin{equation}
%\nu(z)=H_{1}\nu_{1}(z)+H_{2}(z)\nu_{2}(z).
%\end{equation}
...
%   \& \coordinate (aux0);
   ...
   \& \node[rectangle,draw,
	minimum width=1cm,
	minimum height=1cm,
        label=\textbf{Plant}] (gs) {{\sc $G(z) + \Delta_p \vec{G}$}};
...
\caption{System equivalent to the system in Figure~\ref{fig:sampledsystem}\label{fig:digsystem1}}
\end{figure*}
...
\red{[unclear whether this is over the s or z domain. further there is lack of proper subscripts (n,d,s), as per the notations introduced above.]}
%
\begin{equation}
...
$\mathit{FWL}[\cdot]:\mathcal{P}^{n}\rightarrow \mathcal{P}^{n}\langle I,F \rangle$,
which applies the FWL effects to a polynomial.  
\red{above $\mathcal{P}$ was defined as a single polynomial, now it is a space of polynomials. actually space of polynomials form specific algebraic structures, called diods -- we should have used those ... }
$\mathcal{P}^{n}$ is the
...
%
\red{[above you introduced a notation for poly ratios, which now you do not use?]}
Thus, the perturbed controller model $\hat{C}(z)$ may be obtained from the
original model $C(z)=\frac{N_{C}(z)}{D_{C}(z)}$ as follows:
%
\begin{equation}
\hat{C}(z)=\frac{\mathit{FWL}[N_{C}(z)]}{\mathit{FWL}[D_{C}(z)]}.
\end{equation}
%
...
%%
%In particular, Widrow \cite{widrow1956} models the quantisation noise as a
%gaussian model in which the second moment (variance) of the probability
%density function (PDF) of the quantisation noise is given by
%%
%\begin{equation}
...
%%
%The power spectral density of the quantisation noise (supposed white)
...
%\end{proof}
%
%Finally, the quantisation noise spectrum may be considered a energy-bounded
%(Lemma~\ref{lemma:boundednoise}), and consequently the Parseval's theorem
%ensures that the quantisation noise is also energy-bounded in time-domain. 
...
\red{[it's not a lemma, since it is not later used to prove a theorem. state it as a proposition (theorem would be a bit of an over-statement)]}
...
\frac{\mathit{FWL}[N_{C}(z)]}{\mathit{FWL}[D_{C}(z)]}$ and uncertain
discrete model of the plant $\vec{\hat{G}}=\vec{G}+\Delta_p \vec{G}+\Delta_q
\vec{G}$, namely \red{[utter confusion between transfer functions and coefficients - you're next taking the ratio between two vectors of coefficient, which magically now are also a function of the z variable! WTF!  \ldots]} $\hat{G}(z)=\frac{\hat{N}_{G}(z)}{\hat{D}_{G}(z)}$, is
...
S(z)=\mathit{FWL}[N_{C}(z)]\hat{N}_{G}(z)+\mathit{FWL}[D_{C}(z)]\hat{D}_{G}(z);
\end{equation}
\item the direct loop product $\frac{\mathit{FWL}[N_{C}(z)]}{\mathit{FWL}[D_{C}(z)]}\cdot \frac{\hat{N}_{G}(z)}{\hat{D}_{G}(z)}$ has no pole-zero cancellation on or outside the unit circle.
\end{itemize}
\end{mylemma}

Lemma~\ref{lemma:eq_int_stab} provides the necessary conditions for the
ability of a digital controller to stabilize the closed loop system,
considering plant parametric uncertainties (i.e., $\Delta_p \vec{G}$),
quantisation noises ($\nu_{1}$ and $\nu_{2}$) and FWL effects in the control
software.  Based on it, the model for plant uncertainty due quantisation
noise (i.e., $\Delta_q \vec{G}$) may be disregarded if the conditions on
Lemma~\ref{lemma:eq_int_stab} are satisfied and if the quantisation noise is
bounded.  The boundedness of the quantisation noise is
...
Digital Systems Synthsis, named DSSynth.
%
%, which accepts the
...
an ANSI-C input file (Steps 1--3), DSSynth constructs a non-deterministic
...
Finally, DSSynth builds an intermediate ANSI-C code for the digital system
...
property of interst (i.e., robust stability) and is passed to the
...
$\Delta{\vec{G}}=\Delta_p{\vec{G}}+\Delta_b{\vec{G}}$.  % Since this value is
...
is in most cases of several orders of magnitude.
...
increase the precision of \param{I_b}{F_b} and re-start the {\sc synthesise}
phase with an empty {\sc inputs} set.  Otherwise, we found a full solution
for our synthesis problem and we are done.
...
In order to exploit the full potential of the present approach, a
dynamical plant that can present physical components must be
considered.  However, a discrete plant model can also demonstrate some
benefits of the present approach, which considers FWL effects in the
digital controller implementation.  In particular, a classical cruise
control example is extracted from the literature~\cite{Astrom08},
where the discrete plant model (with a time step of $0.2$s) is
...
\end{equation}
%
The authors claim that this controller $C(z)$
(Eq.~\eqref{Eq:running-example-plant}) stabilizes the closed-loop system for
the discrete plant model $G(z)$ (Eq.~\ref{Eq:running-example-controller}). 
However, if implementation aspects are considered during verification (e.g.,
...
phase, where we get the following candidate solution:
%
...
This time, the {\sc uncertainty} check does not find any more
counterexamples and we pass the current candidate solution to the {\sc
...
\resizebox{.8\textwidth}{!}{
 \begin{tikzpicture}[scale=0.3,->,>=stealth',shorten >=.2pt,auto, semithick, initial text=, ampersand replacement=\&,]
  \matrix[nodes={draw, fill=none, shape=rectangle, minimum height=.2cm, minimum width=.2cm, align=center
   ...
   \& \coordinate (aux2);
   \&;\\
   \node[fill=gray!20] (synth) {{\sc Synthesise}};
   \&
   complexnode/.pic={ 
...
\item RQ1 \textbf{(sanity check)} which results does our CEGIS approach to
FWL controller synthesis obtain upon synthesising/verifying benchmarks that
compose the specified suite?

\item RQ2 \textbf{(performance)} does the CEGIS approach provide a feasible
...
is able to find a stable solution in less than one minute.  This is possible if the
...
further conclude that the approach using a two-stage verification engine is
able to synthesise stable controllers in the $12$ out of $15$ benchmarks,
and can be complemented using the one-stage engine for $2$ additional
benchmarks, where the inductive conjectures fail.  Both verification engines
together enabled controller synthesis for $14$ out of $15$ benchmarks.  For
the $1$ remaining benchmark our approach failed to synthesise a stable
controller in the requested time limits.  This can be addressed by either
increasing the fixed point word widths considered or using floating point
arithmetic instead.  A link to the full experimental environment, including
scripts to reproduce the results, all benchmarks and the DDSynth tool, is
provided in the
...
synthesise correct-by-construction digital controllers for a given model of
