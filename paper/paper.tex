...
be disregarded. Additionally, the pulse transfer function obtained via \eqref{eq:pulsetf} using an appropriate sample rate ({\it i.e.}, obeying the Nyquist sampling conditions)  is a precise discrete-time representation of $G(s)$.
%
\end{myassumption}

Although the pulse transfer function $G(z)$ represents adequately $G(s)$ in the z-domain, the model $G(z)$ may not be accurate and some uncertainties may not be covered by that model. These uncertainties may be parametrically expressed by an additive transfer function $\Delta G(z)$, resulting in a uncertain model $\hat{G}(z)$, such that:
\begin{equation}
