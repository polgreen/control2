...
which applies the FWL effects to a polynomial, with an equivalent $\mathit{FWL}_{n,m}[\cdot]:\mathcal{P}^{n,m}\rightarrow \mathcal{P}^{n,m}\langle I,F \rangle$ that apllies the same effect to a ratio of polynomials, where
$\mathcal{P}^{n}$ is the
...
In particular, Figure~\ref{fig:bode} shows the Bode diagram of the closed-loop system from~\cite{DBLP:conf/hybrid/WangGRJF16} 
