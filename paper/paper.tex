...
for all possible uncertainties $\Delta{\vec{p}}$ (\ref{eq:uncertainty}).
%$(\Delta N_G, \Delta D_G)$. 
...
Thus, we compute the coefficients for $N_c$ and $D_c$ in the domains 
$\mathbb{R}^{n_C}\param{I}{F}$ and $\mathbb{R}^{m_C}\param{I}{F}$, respectively, where 
$n_C$ and $m_C$ denote the order of the corresponding polynomials
and $I$ and $F$ denote the integer and fractional size, respectively.
Regarding the uncertainty, this is synthesised over the 
...
\end{table}
\pksay{TODO: @Lucas: Please complete $G_x$ in table.}
%---------------------------------
