...
coefficients $b$ and $a$~\cite{Astrom08} \red{[pls say how]}.  
...
$Q2$) \red{[do we need the second? See my further comments below.]} are white noises with zero mean.
%
\end{myassumption}
...
limit-cycle oscillations. \red{[explain difference btw overflows and saturations. Also, do we care about limit cycles? I'd quite happily disregard them.]}
%
\end{myassumption}
...
%
The pulse transfer function \red{[define PTF]} $G(z)$ is equivalent at the sample times to the
...
For stability, there are only two counterexamples, which are known. 
Thus, we can just enumerate them..
%\cdsay{Then, do we actually need program synthesis?}
