...
%------------------------------------
\subsection{On the stability considering effects of implementation issues and quantization noises}
\label{sec:stability}
%------------------------------------


In order to obtain a sound synthesis of digital controller, the effect of FWL implementation in controller and quantization noises in closed loop system have to be considered. Considering that the quantizer $Q1$ is the source of a white noise with zero mean $\nu_{1}$ and $Q2$ is the source of a white noise with zero mean $\nu_{2}$, the following equation may written for the system in figure~\ref{fig:sampledsystem} considering the parametric uncertainties $\Delta G(z)$ and the FWL effects on controller  ($\hat{C}(z)$):
\begin{equation}
...
The Lemma~\ref{lemma:eq_int_stab} provides the necessary conditions for synthesis of digital controllers able to stabilize closed loop system, considering plant parametric uncertainties ($\Delta G_{P}(z)$), quantization noises ($\nu_{1}$ and $\nu_{2}$) and FWL effects in digital controllers. Based on it, the model for plant uncertainty due quantization noise ($\Delta G(z)$ may be disregarded if the conditions on Lemma~\ref{lemma:eq_int_stab} are attended and if the quantization noise is bounded. 
