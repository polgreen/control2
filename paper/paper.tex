...
non-deterministic model to represent the plant family (Step A), and a transfer
function $f$ represented by Eq.~\ref{FWL_tf} (step B) to calculate the parameters
of the controller to be synthesized (step C).  Finally DSVerifier builds an
intermediate ANSI-C code for the digital system implementation, which is used as
input for the controller synthesiser (Step D).
...
coefficients by the function $f$ (cf.~Eq.~\eqref{FWL_tf}).
