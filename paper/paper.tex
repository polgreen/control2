...
be stable for all plants in the family and only generate counterexamples when the precision of the
engine is insufficient (\emph{ie} no stable plant could be found). Given the high computational cost of this
...
uses a two-stage verification with a first stage that performs unsound fixed point operations given the precision \param{I_b}{F_b}, and a second stage that restores soundness by validating these operations using interval arithmetic on the synthesised plant.
