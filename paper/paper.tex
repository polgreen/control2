...
discrete time and over a quantised domain.  Our approach translates the problem into a single digital domain.  
...
digital controller).  To obtain a joint model, we discretise the continuous
...
\begin{myassumption}
%
The Sample and Hold of the ADC and the ZOH of the DAC are synchronised,
...
Here, Lemma~\ref{eq:pulsetf} ensures the poles and zeros matching with
...
%
A feedback hybrid control system as given in Fig.~\ref{fig:sampledsystem},
...
given in Fig.~\ref{DSSynth_process}.  Steps $1$ to $3$ are performed by
...
reported if CEGIS generates a controllerthat is safe with respect to~$\phi$. 
...
The second approach is illustrated in Fig.~\ref{fig:CEGIS} and uses a
