...
\textcolor{blue}{The digital controller $C(z)$ also suffers disturbances from the FWL effects, with roundoffs in coefficients that may change the closed-loop poles and zeros position, and consequently affect its stability, as shown in~\cite{Bessa16}.}

\textcolor{blue}{Since the controller is implemented in the $\mathbb{R}\langle I,F \rangle$ 
domain, {\it i.e.} a Fixed-Point Finite Word Length with integer size $I$ and fractional
size $F$, we shall denote this domain as $\mathbb{R}\langle I,F \rangle$ and assume that there exists a function $FWL[\cdot]:\mathbb{R}^{n}\rightarrow \mathbb{R}^{n}\langle I,F \rangle$, which applies the FWL effects to a polynomial. Thus the perturbed controller model $\hat{C}(z)$ may be computed from the original model $C(z)=\frac{N_{C}(z)}{D_{C}(z)}$ using the following expression:
\begin{equation}
