...
physical environment (the plant), which are {\em stable}.  Due to the complexity
...
set in the representation of these coefficients by the function $f$ (cf. Eq. \eqref{FWL_tf}).
...
%
Indeed, the actual implementation of $C(z)$ using \param{4}{16} implementation format 
...
First, some assumptions have to be established to obtain a model for the quantization effects (seen as noise) on the closed loop system. 
%
\begin{myassumption}
\label{whitenoise}
%
...
In \cite{widrow1956}, Widrow models the quantization noise as a gaussian model in which
the second moment (variance) of the probability density function (pdf) of the quantization
noise is given by
%
\begin{equation}
...
\end{equation}
where $q$ is the step size of quantizer, thus supporting this assumption.
\end{myassumption}
...
\hat{Y}(z)=H_{1}(z)\left(R(z)+\nu_{1}(z)\right)+H_{2}(z)\nu_{2}(z)
\end{equation}
$$H_{1}(z)=\frac{C(z)G(z)}{1+C(z)G(z)}$$
$$H_{2}(z)=\frac{G(z)}{1+C(z)G(z)}$$

In the case of stability properties we need to verify the stability of both $H_{1}(z)$ and $H_{2}(z)$ to validate the stability of the whole system.
...
$\exists \vec{P} . \forall \vec{x} \in \text{\sc inputs} . \sigma(\vec{x}, \vec{P})$
where $\vec{P}$ ranges over functions, $\vec{x}$ ranges over
ground terms and $\sigma$ is a quantifier-free formula.
We interpret the gound terms over some finite domain $\mathcal{D}$.
...
tries to find an existential witness $P$
...
loop adds a new input to the finite set $\text{\sc inputs}$  being used for synthesis.  
Given that the full set of inputs $\mathcal{D}$ 
...
%% \red{[unclear: what matrix? We need to clarify the relationship between the char. poly. $S(z)$ and the later quantities $\Delta N_{G}(z), \Delta D_{G}(z)$ and corresponding controller's structure.]}.
%
...
This section is split into three parts. Section~\ref{experimental-setup} discusses 
the experimental setup and the employed benchmarks, while Section~\ref{experimental-objectives} 
describes the experimental objectives. Section~\ref{experimental-results} describes the experimental 
results related to the application of CEGIS to the standard closed-loop control system benchmarks 
described in Section~\ref{experimental-setup}.
