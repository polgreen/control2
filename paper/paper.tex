...
\maketitle
\begin{abstract}
Modern control systems are by and large implemented via digital microcontrollers, 
which are designed to work in an embedded environment, 
within a dynamical plant that can present physical components. 
%
This poses a new dimension to the problem of controllers synthesis,   
which is classically studied in digital control literature. 
%
In this study, we plan to provide a principled approach to extend this literature in a number of directions: 
we are interested in quantitative tasks;  
we focus on automated correct-by-design synthesis algorithms, as opposed to verification objectives; 
and we encompass the compete range of approximations, related to time discretisation, quantisation effects, 
and finite-precision representations (with related rounding errors). To this end, 
we propose a CEGIS-based solution that uses inductive invariant synthesis in conjunction with a digital verification algorithm 
to find a parametric solution for a digital controller given a continuous plant specification. 
\end{abstract}
...
correct behaviour in a number of different environments.  % For this reason it is
...
systems~\cite{istepanian2012digital}, i.e. the ZOH discretization models the exact effect of sampling and DAC interpolation over the plant.% {\item \red{[but the ZOH is for the DAC, not for the ADC?]}.
...
Consider the closed-loop sampled system in Figure~\ref{fig:sampledsystem} \red{[requires updating]},
...
converter (DAC).  
\red{[do we really need to regard the quantisation effects of the ZOH in the DAC? Elaborate next:]}
$G(s)$ is the continuous-time model of the plant, from
...
%   \& \coordinate (aux0);
 ...
 \caption{Sampled System. \label{fig:sampledsystem} \red{[Please update this figure: reference enters downstream of control block. Clarify signals over lines in plot.]}}
\end{figure}
...
$Q2$) \red{[do we need the second? See my further comments below.]} \green{There was a typo in \eqref{eq:deltaG_final}. It was missing $q_{1}$ and $q_{1}$, both effects are considered here. } \red{[why are both needed? I'm still unconvinced.]}
are white noises.
%
\end{myassumption}
...
\begin{myassumption}
%
The controller and the plant are linear, {\it i.e.} any source of nonlinearity ({\it e.g.} overflows) may be disregard. %\red{[explain difference btw overflows and saturations. Also, do we care about limit cycles? I'd quite happily disregard them.]}
%
\end{myassumption}
...
\end{mylemma}
%\red{[Here the dependence on the sampling time T needs to appear somewhere.]} 
%\green{It appears,  $kT$ is the time $t$ used after the inverse Laplace transform}
%\red{[ok, then let's call it $G(z,T)$ as I suggested earlier.]}
...
\blue{[Perhaps we should anticipate this example, partly showing potential repercussions of our formal approach. 
Indeed, this example does not show the full potential, since it does not start with the physical plant $G(s)$. ]}
...
the work in~\cite{DBLP:conf/hybrid/WangGRJF16} has designed a high-performance controller that is characterised by the following z-domain transfer function:
\begin{equation}
\label{Eq:running-example-controller}
C\left(z\right) := \frac{2.72z^2 - 4.153z + 1.896}{z^2 - 1.844z + 0.8496}.
\end{equation}
The authors claim that this controller $C(z)$ is extremely stable for the discrete plant model $G(z)$.  
However, if \red{[which ones? quantisation?] implementation aspects} are considered during verification, 
this controller becomes unstable.
%
Indeed, the actual implementation of $C(z)$ using \param{4}{16} \red{[clarify this notation]} 
implementation format can be represented by the following expression: 
\begin{equation}
...
\end{equation} 
which makes the system unstable in closed-loop \red{using the typical series loop configuration [meaning?]}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\red{Automated} Program Synthesis for \red{for Controllers}}
...
one based on Jury's criteria \cite{DBLP:journals/dafes/BessaICF16} \red{[Jury here is our friend, I wouldn't use his first name ... ]}.
...
\resizebox{.7\textwidth}{!}{
 ...
 \caption{Counterexample-Guided Inductive Synthesis of Programs (CEGIS paradigm). \label{fig:CEGIS}}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\red{[All very interesting, however we never start from a $G(s)$ plant, we never really display the whole capabilities of the technique! Or am I missing something?]}
