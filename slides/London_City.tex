...
\only<2->{
  ...
  & I(x) \wedge G(x) \wedge B(x, x') \rightarrow I(x') ~ \wedge \\
...
\only<3->{
  Also: finding bugs, termination, non-termination, superoptimisation, refactoring,
...
\item Genetic programming
    \vspace{1cm}
\pause
\item Future work: machine learning based techniques  
\end{itemize}
...
\begin{itemize}
\item Employs abstract
acceleration to evaluate all possible progressions of the system simultaneously.
\vspace{1cm}

\item Makes use of the abstract accelerator described in: ``Unbounded-time analysis
