...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \title{\fontsize{11}{12}\selectfont Solving Second-Order Constraints with Program
Synthesis}
%\subtitle{CAV 2017}
...
\author{\normalsize Alessandro Abate, Iury Bessa, Dario Cattaruzza,\\
  {\bf Lucas Cordeiro}, {\bf Cristina David}, Pascal Kesseli,\\
  Daniel Kroening, Matt Lewis and Elizabeth Polgreen}
...
\begin{frame}[fragile]{What is this talk about?}
%\begin{definition}[Synthesis Fragment ($SF$)] \label{def:synthesis-frag}
...
\centering
Solving this formula.  %We'll be talking about it a lot.

%
%where the $P_i$ range over functions, 
%the $Q_i$ are either $\exists$ or $\forall$,
%the $x_i$ range over ground terms and $\sigma$ is a quantifier-free formula.


%$SF$ is implicitly
%defined over some background first-order theory, and $\sigma$ must be
%computable in that theory.
% - over finite domain, it is decidable
% - NEXPTIME-complete
\end{frame}

\begin{frame}[fragile]{Why solve $\exists F . \forall x . \sigma(F, x)$?}
\vspace{-.5cm}
...
\begin{align*}
\only<2>{
  \exists I . \forall x, x' .  & Init(x) \rightarrow I(x) ~ \wedge \\
  & I(x) \wedge G(x) \wedge T(x, x') \rightarrow I(x') ~ \wedge \\
  & I(x) \wedge \neg G(x) \rightarrow A(x)
%\end{align*}
}
%\begin{align*}
\only<3->{
  \exists N, x_0 . \forall x . \exists x' . & Init(x_0) \wedge N(x_0) ~\wedge \\ 
&  N(x) \rightarrow G(x) ~ \wedge \\
& N(x) \rightarrow T(x, x') \wedge N(x') \\\\
 %% \exists R . \forall x, x' .  & G(x) \rightarrow R(x) > 0 ~ \wedge \\
 %%  & G(x) \wedge B(x, x') \rightarrow R(x) > R(x')
}
%\end{align*}
\only<4-> {
% \begin{align*}
  \exists N, {\color{red} S}, x_0 . \forall x . & Init(x_0) \wedge N(x_0) ~\wedge \\ 
&  N(x) \rightarrow G(x) ~ \wedge \\
& N(x) \rightarrow T(x, {\color{red} S(x)}) \wedge N({\color{red} S(x)})
} 
\end{align*}
...
\only<5->{
  Also: finding deep bugs, termination, superoptimisation, refactoring,
  synthesising digital controllers\\
}
%% \only<5>{
%% Existing solvers: E-HSF for forall-exists Horn clauses ({\scriptsize ``Solving Existentially Quantified Horn Clauses'', Beyene, Popeea, Rybalchenko, CAV'13}). 
%% % (based on predicate abstraction and interpolation).
%% }
\end{frame}
...
\section{Program Synthesis Based Solver}



\begin{frame}[fragile]{What does a solution look like?}
...
\vspace{2em}
 ...
 %% \visible<3->{
 %%  $F$ is an infinite object -- it's too big to write down!
 %% }
...
\pause
 Instead of directly dealing with functions, we search for \emph{programs} that compute the functions in our solution.
...
\begin{frame}[fragile]{CounterExample Guided Inductive Synthesis}
...
%\begin{figure}
{
 ...
 \begin{tikzpicture}[scale=0.4,->,>=stealth',shorten >=1pt,auto,
 semithick, initial text=]

  \matrix[nodes={draw, fill=none, scale=.8, shape=rectangle, minimum height=1cm, minimum width=1.5cm},
          row sep=1.5cm, column sep=2.5cm] {
...
\vspace{3em}
Synthesise: $\exists P . \forall x \in \text{\sc inputs} . \sigma(P, x)$

\vspace{1em}
Verify: $\exists x . \lnot \sigma(P, x)$
...
\begin{frame}{Program generation strategies}

\begin{itemize}
\item Symbolic Bounded Model Checking
  \vspace{1cm}
\item Genetic Programming
    \vspace{1cm}
\pause
\item Future work: Neural network  
\end{itemize}
  
\end{frame}

\begin{frame}[fragile]{Symbolic Bounded Model Checking}
\vspace{-.5cm}
\centering
\begin{minipage}[t]{.45\textwidth}
\begin{lstlisting}[language=C++,basicstyle=\small,mathescape=true]
void synthesise() {
  prog_t p = nondet();
  int out[M];

  exec(p, input$_1$,out);
  assume(check(input$_1$,out));
  ...
  exec(p, input$_n$,out);
  assume(check(input$_n$,out));
...
Search through the space of programs
for an individual that is ``fit'' in some sense.
%(we measure fitnes by the number of INPUTS for which it satisfies the specification).
\vspace{1cm}

Evolve the population of programs:
\begin{itemize}
  \item CROSSOVER combines selected existing programs
    into new ones
    \item MUTATE randomly changes parts of a single program
\end{itemize}
      
\end{frame}

\begin{frame}[fragile]{Searching the program space}
 The space of programs is very large.
 
 \vspace{2ex}
 
 In order to search it efficiently, we parametrise the space in various dimensions (program length, word width, number of constants)
 and systematically increase these parameters to cover the whole space.
 
 \vspace{2ex}
 
 An important strategy is \emph{generalisation} -- we find simple solutions that solve a restricted case of the specification, then
 try to generalise to a full solution.
 
 \vspace{6ex}
 
 {\scriptsize More details in the paper: ``Using Program Synthesis for Program Analysis'', LPAR'15, David, Kroening, Lewis}
\end{frame}


\begin{frame}{Synthesis of digital controllers [CAV'17]}
...
%% TODO: add something about Jury's criterion.
...
%  \includegraphics[width=.25\textwidth]{figures/cav2017frontpage.pdf} 
%\begin{tabular}{l}  
%\hspace{-4cm}

\begin{itemize}
\item Employ abstract
acceleration to evaluate all possible progressions of the system simultaneously.


\item Made use of the abstract accelerator described in: ``Unbounded-time analysis
  of guarded LTI systems with inputs by abstract acceleration'', SAS'15, Cattaruzza, Abate, Schrammel, Kroening.
