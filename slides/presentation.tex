...
\textcolor{red}{Copy here CEGIS architecture scheme.}
...
\resizebox{1.0\textwidth}{!}
{
 \begin{tikzpicture}[scale=0.3,->,>=stealth',shorten >=.2pt,auto, semithick, initial text=, ampersand replacement=\&,]
  \matrix[nodes={draw, fill=none, shape=rectangle, minimum height=.2cm, minimum width=.2cm, align=center
   ...
   \only<1-6,8-12,16,18-19>{\path ([yshift=-2em]verif1.west) edge node {Counterexample} ([yshift=-2em]synth.east);}
...
\begin{frame}[fragile]{Completeness Bound}
\begin{figure}[t]
\centering
...
\begin{tikzpicture}[scale=0.3,->,>=stealth',shorten >=.2pt,auto, semithick, ampersand replacement=\&,]
  ...
  \only<1-3>{\path (abstract.south) edge node{$\textcolor{blue}{\begin{array}{c}\phi_{init}^K\\P_a(z)\\k=0\end{array}}$} (synth.north);}
  \only<4->{\path (abstract.south) edge node{$\textcolor{blue}{\begin{array}{c}\phi_{k,x_0}^K\\P_a(z)\\k=0,1,2\end{array}}$} (synth.north);}
    \only<1,4>{\path ([yshift=1em]synth.east) edge node {$K$} ([yshift=1em]verify.west);}
    \only<2-3,5->{\path ([yshift=1em]synth.east) edge node {$\textcolor{blue}{K}$} ([yshift=1em]verify.west);}
    \path (aux) edge (abstract.east);
  \only<1-3>{\path ([xshift=+.6cm]KSAT.north) edge[left] node[xshift=-.3cm] {$\textcolor{blue}{\begin{array}{c}\phi_{init}^K\\P_a(z)\end{array}}$} ([xshift=0.6cm]synth.south);}
  \only<4->{\path ([xshift=+.6cm]KSAT.north) edge[left] node[xshift=-.3cm] {$\textcolor{blue}{\begin{array}{c}\phi_{k,x_0}^K\\P_a(z)\\k=0,1,2\end{array}}$} ([xshift=0.6cm]synth.south);}
    \only<1>{\path ([xshift=-.6cm]synth.south) edge node[align=center,xshift=.3cm] {$K$} ([xshift=-.6cm]KSAT.north);}
    \only<2-3>{\path ([xshift=-.6cm]synth.south) edge node[align=center,xshift=.3cm] {$\textcolor{blue}{\left[\begin{array}{c}0.24609375\\-0.125\\0.1484375\end{array}\right]^T}$} ([xshift=-.6cm]KSAT.north);}
    \only<4>{\path ([xshift=-.6cm]synth.south) edge node[align=center,xshift=.3cm] {$K$} ([xshift=-.6cm]KSAT.north);}
    \only<5->{\path ([xshift=-.6cm]synth.south) edge node[align=center,xshift=.3cm] {$\textcolor{blue}{\left[\begin{array}{c}0.23828125\\-0.17578125\\0.109375\end{array}\right]^T}$} ([xshift=-.6cm]KSAT.north);}
    \only<1-2,4>{
  ...
  \only<1-2,5->{\path[-] (verify.north) edge node {$(k,x_0)$} (aux);}
  \only<3-4>{\path[-] (verify.north) edge node {$\tiny \textcolor{blue}{\left(\begin{array}{cc}2,&[\begin{array}{ccc}.9&-.9&.9\end{array}]\\3,&[\begin{array}{ccc}.9&-.9&-.9\end{array}]\end{array}\right)}$} (aux);}
\end{tikzpicture}
}
\end{figure}
\end{frame}
