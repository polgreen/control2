...
\begin{frame}{Transfer-Function Representation}
%
\begin{equation}
\small
...
\item $N$ and $M$ are the \textbf{order} of the \textbf{denominator} and \textbf{numerator} polynomials, respectively.
\end{itemize}

\end{frame}

\begin{frame}{State-Space Representation}
In state-space form, the behavior of a system is represented via a \textbf{state
...
\begin{frame}{DSSynth Architecture}
...
\begin{enumerate}
\item \textit{plant} is the physical plant in state-space or transfer function representation;
...
\begin{frame}{DSSynth Usage - Illustrative Example}

\colorbox{green!70}{Synthesizing a unmanned aerial vehicle (UAV) plant:}
...
\colorbox{green!70}{Controller Synthesized:}
...
\colorbox{green!70}{Step response to check stability:}

\begin{figure}[ht]
  \includegraphics[width=0.5\textwidth]{figures/step-response.eps}
  \caption{Step response for UAV Plant describing a stable system.}
  \label{step-response}
\end{figure}
...
\begin{itemize}
\item The signal input range is between $-1$ and $1$.  
\item  The implementation features used for digital controller are: 
$8$~bits for the integer part and $8$~bits for the fractional part.  
\item  We~have run the DSSynth using CEGIS with multi-staged verification.
\end{itemize}
...
\begin{itemize}
\item Evaluate the DSSynth performance to produce stable and safe controllers, and also
...
\begin{itemize}
\item The median runtime for our benchmarks is $124$\,s;
\item The average synthesis time amounts to $35.5$\,s for state-space systems;
\item For transfer functions, the average synthesis time amounts to $123.6$\,s.
\end{itemize}
\end{block}
...
\item {\bf Automated synthesizer for state-space and transfer-function representations that ensures stability and safety;}

\item {\bf DSSynth presents a practical application of CEGIS-based program synthesis;}

\item {\bf Given the current literature in controller synthesis, there is no other MATLAB toolbox for synthesizing stable and safe digital controllers
for physical plants that considers all aspects of their digital implementation.}
