...
\begin{itemize}
\item support for transfer-function and state-space representations in closed-loop form
\item synthesize different numerical representations of the controller using CEGIS
\item provide a MATLAB toolbox to synthesize digital controllers while taking into account finite word-length effects
\end{itemize}
...
\item The number of {\bf state variables} ranges from 1 to 9
\item The {\bf median runtime} for our benchmarks is $124$\,s
\item The average synthesis time amounts to $35.5$\,s for state-space systems
\item The average synthesis time amounts to $123.6$\,s for transfer functions
\item On average our engine spent 52\% in the synthesis and 48\% in the verification phase
\end{itemize}
\end{block}
