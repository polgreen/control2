...
\begin{center}
  \begin{overprint}
    \onslide<1>
    \centerline{\includegraphics[width=.8\textwidth]{figures/spirals/diagram-15.png}}
    \onslide<2->
    \centerline{\includegraphics[width=.8\textwidth]{figures/spirals/diagram-16.png}}
  \end{overprint}
  \end{center}
  
  \only<1>{{\bf Continuous-discrete system}}
  \only<2->{{\bf Continuous system}}  
  \begin{itemize}
  \item \only<1>{\colorbox{green!70}{Plant:
    \textsc{$\dot{x}(t) = Ax(t)+ Bu(t), \quad t \in \mathbb R_0^+,~x(0)=initial~state$}}}
    \only<2->{\colorbox{azure!70}{Plant:
  ...
  \item \uncover<3->{Finite precision}  
\end{itemize}
\end{frame}
...
\vspace{-1cm}
  \begin{center}
    \only<1>{\includegraphics[width=.8\textwidth]{figures/spirals/diagram-18.png}}
    \only<2>{\includegraphics[width=.8\textwidth]{figures/spirals/diagram-19.png}}
    \only<3->{\includegraphics[width=.8\textwidth]{figures/spirals/diagram-20.png}}
  \end{center}
  
  \begin{itemize}
  \item \uncover<1->{Truncation and rounding on the plant}
  \item  \uncover<2->{Truncation on the converters}
  \item \uncover<3->{Rounding on the controller}
  \end{itemize}  
...
\begin{overprint}
  ...
  \onslide<2>
  \centerline{\includegraphics[width=.9\textwidth]{figures/spirals/diagram-27.png}}
  \onslide<3>
  \centerline{\includegraphics[width=.9\textwidth]{figures/spirals/diagram-28.png}}
\end{overprint}
...
\begin{frame}{Stability \only<5>{and safety}}
  ...
  asymptotically stable if its model
  ...
  \end{tabular}
   ...
   \hspace{1cm}
     \begin{minipage}{0.6\linewidth}
      \begin{algorithm}[H]
\begin{algorithmic}[1]
        \STATE \textbf{Input:} $x_0$, $k$.
        \STATE \textbf{Output:} {\color {red} $C$}.
       \STATE {\color {red} $C$}=nondet;
       \STATE \textbf{assume}({\color {black} \textbf{STABLE}}({\color {red} $C$}));
        \STATE \textbf{assume}({\color {black} \textbf{SAFE}}($x_{0}$));       
  \FOR{$i=0$ to $k-1$}
