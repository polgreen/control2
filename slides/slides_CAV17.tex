...
\subtitle{CAV 2017}
\institute{\normalsize University ~of ~Oxford, \\Diffblue Ltd.,\\ Federal University of Amazonas}
\date{}
\author{\normalsize Alessandro Abate, Iury Bessa, Dario Cattaruzza, Lucas Cordeiro, {\bf Cristina David}, Pascal Kesseli, Daniel Kroening and Elizabeth Polgreen}
...
\begin{center}
  \begin{tabular}{lll}
    \includegraphics[width=.2\textwidth]{figures/thermostat.jpg} &
...
\uncover<2>{
\vspace{.5cm}
{\bf Automatically synthesise feedback digital controllers that make cyber-physical systems stable and safe}.}
...
{\bf Safety:} (1) incremental unfolding (2) abstraction}
...
\begin{frame}{Iterative unfolding-based controller synthesis} %%Na\"ive Approach: CEGIS with multi-staged verification}
...
\resizebox{.65\textwidth}{!}
{
 ...
 \begin{tabular}{p{\dimexpr 0.5\linewidth-2\tabcolsep} 
   ...
   \only<2>{
     \begin{tabular}{l}
       {\bf Find a controller for given $\{x_0\}$, $k$ and} \\
       {\bf plant precision such that the system is}\\
       {\bf stable and safe.} \\
%       {\bf states $x_0$, a given number of discrete steps $k$,} \\
 %      {\bf and a given precision $\langle I_p, F_p \rangle$.}\\
  ...
  \end{tabular}
   ...
   \only<4>{
     \begin{tabular}{l}
       {\bf Find a controller for $\{x_0, x_0'\}$, $k$, and}\\
       {\bf plant precision such that the system is}\\
       {\bf stable and safe.} \\
%       {\bf states $x_0$, a given number of discrete steps $k$,} \\
 %      {\bf and a given precision $\langle I_p, F_p \rangle$.}\\
...
\end{tikzpicture}
 ...
 \begin{tabular}{p{\dimexpr 0.5\linewidth-2\tabcolsep} 
   ...
   \only<2>{
     \begin{tabular}{l}
       {\bf Find controller for given $\{x_0\}$, $\{k_1\}$ and}\\
       {\bf plant precision such that the system is}\\
       {\bf stable and safe.}
     \\\\\\\\\\\\\\\\\\\\\\\\\\\\
  \end{tabular}
  ...
  \end{tabular}
   ...
   \only<7>{
     \begin{tabular}{l}
       {\bf Find controller for given $\{x_0, x_0'\}$, $\{k_1, k_2\}$}\\
       {\bf and plant precision such that the system}\\
       {\bf is stable and safe.}
     \\\\\\\\\\\\\\\\\\\\\\\\\\\\
  \end{tabular}
   }
   \only<8>{
...
%
\hline
\# & \multicolumn{1}{|c|}{Benchmark} & \multicolumn{1}{|c|}{Order} & \multicolumn{2}{|c|}{Iterative unfolding}                 & \multicolumn{2}{|c|}{Abstraction} \\
